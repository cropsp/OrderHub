...
backfill (`POST /api/shops/{shop_id}/backfill-platform-fees`, dry-run first — its
`overlapping_settlements` will legitimately list the Kseniia settlement; informational, since ⑤
deletes and recreates it) → ⑤ Kseniia config + resettle; OWNER-vs-MANAGER gating with a real
manager token remains unexercised; docs commit + push.
