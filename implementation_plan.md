...
**Follow-ups filed below:** `ORDER-SHIPPING-2` (closed, next entry), `SHOPIFY-VERSION-DRIFT`,
`WEBHOOK-MAPPER-DEGRADED`, `SHOPIFY-HISTORY-GAP`, `PROD-LOGS-EPHEMERAL`.

---

### ORDER-SHIPPING-2 — shipping-line discounts, and the backfill actually run (closed 2026-08-05)

**Merged fast-forward `b7a74a5` → `37ce327`, deployed to prod 2026-08-05.** Migration
`b8d3f0a25c47`, single head. **The backfill has been run — one real write, 445 orders.** This entry
closes the whole ORDER-SHIPPING line: the manager now sees what an order total is made of, on new
orders and on history alike.

**Problem.** `totalShippingPriceSet` is the **pre-discount** shipping charge and `totalDiscountsSet`
**includes** shipping discounts, so ORDER-SHIPPING-1 would have stored shipping the customer never
paid and booked a shipping promo as an item discount. Two orders on prod, both 2026-07:
`91890_1815` (49.00 → 0.00) and `91890_1829` (54.00 → 0.00), each a 100% `SHIPPING_LINE` discount
with no item discount at all.

**Fixing `shipping_revenue` alone would have been worse than shipping the bug.** The card renders
`items − discount + shipping + tax = total`; on `91890_1815` that is `369.29 − 49.00 + 49.00` today
(wrong but balanced), `369.29 − 49.00 + 0.00 = 320.29` with a shipping-only fix (wrong **and**
visibly broken), and `369.29 − 0.00 + 0.00` with both corrected. The two derived values had to move
together.

**What shipped.** A fourth nullable `Numeric(10,2)` column, `shipping_discount`, and the four-value
rule:

```
shipping_revenue  = Σ shippingLines[].discountedPriceSet
shipping_discount = Σ (originalPriceSet − discountedPriceSet)
discount_total    = totalDiscountsSet − shipping_discount     (item-level only)
tax_total         = totalTaxSet
```

`discount_total` changed meaning — it is now the **item** discount, and the card labels it so.
Suites 823 backend / 177 frontend; `tsc` byte-identical to `main`; migration round-tripped.

**Decisions worth preserving:**

- **The backfill's fill rule was narrowed mid-implementation, against the approved plan.** "Any
  column NULL, per column" can produce a contradictory row: an order imported *between* the two
  deploys holds gross shipping and a shipping-inclusive discount, so filling only `shipping_discount`
  with a non-zero value would leave the row asserting both that shipping was charged in full and
  that it was given away. The rule now fills such a row **only when the fresh `shipping_discount`
  is 0.00**, where old and new mappings provably agree (`original == discounted` ⇒ gross == net, and
  `totalDiscountsSet − 0` ⇒ the old value). Non-zero is refused and surfaces as drift. Both branches
  are pinned as tests. **The window never fired** — no order was created between the deploys, so all
  445 rows were a clean slate and the refuse branch has never executed in production. Note its
  standing cost: a refused row stays wrong permanently, because the next run refuses identically.
  The cleaner long-term answer is that these four columns are channel-reported facts with no human
  input to protect, so the backfill could overwrite them unconditionally from source — see
  `ORDER-SHIPPING-OVERWRITE` below.
- **`SHOPIFY_API_VERSION` pinned `2024-04` → `2025-10`, behaviourally a no-op.** See the
  ORDER-SHIPPING-1 entry for why the old pin was inert. Confirmed on the wire after deploy: the
  first live sync logged `POST .../admin/api/2025-10/graphql.json 200 OK` — which simultaneously
  proved the enlarged `ORDERS_QUERY` carrying `shippingLines { originalPriceSet discountedPriceSet }`
  parses, is authorised under the prod token's scopes, and stays inside the cost budget. That was
  the one thing local tests could not establish.

**Verification — three runs, all figures identical.** Dry run 13:49, real write 14:19, post-write
dry run 14:20. The write payload verbatim:

```
matched 445 · found_in_shopify 445 · missing_in_shopify 0 · targets 445 · updated 445
USD: shipping 4663.40 · shipping_discount 103.00 · discount 564.14 · tax 54.34
2026-01   4 ord ·   36.00 /   0.00 /   0.00 /  0.00      2026-06  35 ord ·  354.20 /   0.00 /   0.00 /  2.66
2026-03  52 ord ·  488.00 /   0.00 /  67.75 /  2.45      2026-07  93 ord · 1037.70 / 103.00 / 163.82 / 18.89
2026-04  89 ord · 1015.50 /   0.00 / 211.78 /  2.45      2026-08  25 ord ·  260.70 /   0.00 /   0.00 /  0.00
2026-05 147 ord · 1471.30 /   0.00 / 120.79 / 27.89
unbalanced [] · drift all 0 · drift_samples []
```

Read back independently from the DB rather than trusting the report: `4663.40 / 103.00 / 564.14 /
54.34` over 445 rows — equal to the cent. Census by shop: Lamamarka Shopify 445 with all four set;
Lamamarka ETSY 219 and Vine&Roses ETSY 17 still NULL, untouched. The post-write dry run returned
`targets 0` — the fill-only guard now matches nothing, so re-running is a strict no-op, **proven
rather than asserted**.

**The reconciliation, and the four deltas that are now permanent.** Discounts and tax tie to
Shopify analytics **exactly, every month and store-wide**. Order counts tie every month, with
2026-04 at +1 and only +1. Shipping carries exactly four deltas summing to the store-wide +40.50 —
so nothing hides in a month that nets out:

| Month | Δ | Cause |
|---|---|---|
| 2026-03 | +10.00 | `91890_1457` refunded in-month |
| 2026-04 | +14.50 | `91890_1559` is `test: true`; analytics excludes it, we do not |
| 2026-05 | +8.00 | `91890_1699` refunded in-month |
| 2026-07 | +8.00 | `91890_1660` refund reversal (created May, refunded July) |

**These are definitional, not defects** — we capture sale-time gross, analytics books a refund in
the month it happened. Recorded here so a future reconciliation does not re-investigate them. The
proof that this sprint worked is 2026-07: shipping 1037.70 and discounts **163.82 tying exactly**,
down from the pre-fix 1140.70 / 266.82, with all 103.00 of `shipping_discount` sitting in that one
month and nowhere else.

**Two epistemic notes, both worth more than the numbers.**

- **`drift: 0` was a blind instrument until the write.** Drift is measurable only against a stored
  non-NULL value; with every column NULL the counters were structurally unable to fire, so their
  zeros in the dry run meant nothing. The post-write run is the first where they could fire, and
  they came back 0 on all four — a fresh Shopify read agreeing with every stored figure. This is the
  same class of blind instrument as the ShopifyQL sampling that produced the original wrong premise
  (see the ORDER-SHIPPING-1 entry); recognising it a second time, in one's own work, is the harder
  half.
- **`total_price` drift is 0 of 445** — that column *was* populated, so this counter was live
  throughout. It is the first real measurement of `BUG-4` (stale `order.total_price`) and it comes
  back clean for Shopify orders. Not closure — Etsy and manual orders are unmeasured — but it is
  evidence where there was none.

**Invocation, and what it cost.** All three runs used a temporary script in the prod backend
container calling `backfill_shipping_breakdown` directly, not `POST /shops/{id}/backfill-shipping`.
The HTTP route was weighed and deliberately skipped: its `require_role(OWNER)` is symbolic when the
operator *is* the owner, and the durable-log argument for it is half-empty because that log dies at
the next rebuild (`PROD-LOGS-EPHEMERAL`) while this document is the actual record. The two real
costs were paid explicitly: the script called `logger.setup_logging()` so the service's audit line
landed in the real log (`ORDER-SHIPPING backfill … updated=445 … dry_run=False`, preserved at
`~/server.log.post-backfill`), and it resolved the shop through the router's own conditions
(active / `SHOPIFY` / credentials present). **CC added an undisclosed-then-disclosed guard**: the
commit was made conditional on `updated == targets and unbalanced == []`, rolling back otherwise —
sound, since the backfill is idempotent and a rollback costs nothing while a wrong commit is the
only irreversible outcome. It passed. **Consequence to carry:** the endpoint has still never
executed in production; the next person to need it will be its first user. Filed below.

**Follow-ups filed:** `ORDER-SHIPPING-OVERWRITE`, `BACKFILL-ROUTE-UNEXERCISED`.
...
**Follow-up sprint:** `WB-TRACK-2` (closed, next entry). **Follow-ups filed:** `WB-TRACK-1-followup-1`. `NP-FIX-5` restored below — **this sprint leaves it
...
### WB-TRACK-2 — the parcel monitoring page (closed 2026-08-06)

**Merged fast-forward, `main` = `086e713`, deployed to prod 2026-08-06.** No migration; alembic head
stayed `c7e1b4d93f28`, verified unmoved. Rollback images tagged **by commit** —
`orderhub-{backend,frontend}:rollback-a6e69e6` → `8257ba8c9064` / `4f0c9bf66493`.

`WB-TRACK-1` shipped tracking with no way to see it: grepping the frontend for the endpoint returned
nothing but Tailwind classes. `/westernbid` still showed WesternBid's own `Status`, which reads
`Parcel created` on a parcel that has been moving for a week. This closes the line — the page now
shows 20 parcels needing attention instead of 84.

**What shipped.** `/westernbid` reworked into an exception-first monitor: **Needs attention** (open,
never paginated, ordered problem → no_data → worst-overdue), **Untracked — check by hand** (open),
**In transit** (collapsed), **Delivered** (collapsed, lazy, paged). Rows expand to the NP transition
log, the delivery detail and WesternBid's demoted leg. `Last polled …` sits in the header beside a
`Refresh now` button. Backend additions are additive: `state` accepts a comma-separated set plus
`limit`/`offset` (both default off, so `check_parcel_delivery` sees exactly what it saw before),
`GET /tracking/{n}/events`, `POST /tracking/refresh`. Verified live: 20 + 7 + 33 + 24 = 84.

**OQ1 was made a gate before any component was written, and it earned that.** It did not turn up a
missing field — it turned up a **functionally empty group**. `tracking_number` is the *Nova Poshta*
number, which UPS and USPS parcels do not have, so all seven untracked parcels returned `null`: the
page would have told an operator to "check these by hand" while showing nothing to check with. The
carrier numbers were in `wb_parcel.tracking_numbers` all along and simply never reached the wire. Six
are now actionable (five UPS `1Z08W335D…`, one USPS/ConsolidationOptimum `9261…`); the seventh is the
cancelled parcel, for which WesternBid never issued a carrier number at all — it renders
`WBX260000559260` with an inline `Parcel canceled`, which is exactly the case rule 1 exists to
preserve, handled with no special branch. **The same fix reached the MCP surface**, so the agent can
now name what to check by hand too.

**The page shipped WITH tests — 177 → 196 vitest, two new files.** This is the first UI-touching
sprint since `STATEMENT-UI-TESTS` was filed that added no new UI debt, and it matters more than
usual here: `PROD-BUILD-NO-TYPECHECK` means the prod frontend image is built with no `tsc` at all,
so tests plus a clean local typecheck are the only gate that exists. `tsc` 21 and lint 88, both
exactly at the `TYPECHECK-1` baseline.

**The scheduler fix, folded in deliberately, and what it immediately found.** `run_wb_tracking_poll`
was registered as `interval, days=1` with **no `next_run_time`** — unlike the FX job a few lines
below, which sets one and documents why — so **every backend restart pushed the poll out by up to 24
hours**. Prod had therefore never run a scheduled poll: all 77 rows came from a one-off during the
`WB-TRACK-1` deploy. Shipping a monitoring page on top of a job whose freshness is a function of how
often we deploy is not a defensible state, so the one-line fix went in as its own commit. It paid
for itself at once — the startup poll covered ~17 unpolled hours: `polled=56 changed=13`, three
parcels delivered, six gone dark. (While copying the pattern, the two comment blocks turned out to
have drifted apart — the FX rationale was sitting above the tracking job and the FX job had none;
both were put back with the code they explain.)

**A stub code we had never seen, caught structurally rather than by value.** The six that went dark
returned `StatusCode` **122**; the only stub code known until then was 80. `is_no_data` tests the
*shape* of the response, not the code, so it classified them correctly without knowing 122 exists —
the `WB-1` discipline of never building logic on an undocumented value set, paying off in the one
place it could not have been anticipated. Both `no_data` branches are now live and visibly
different: `59500007112662` has no status at all ("no status was ever recorded", because prod first
saw it already stubbed) while the six code-122 ones kept their last resolved status. Five minutes
later a manual refresh wrote 8 more events, all code **115** — one of the two customs codes absent
from Nova Poshta's developer portal. A batch had cleared customs between polls.

**Two questions the live page raised that the design could not have** — both recorded rather than
acted on, and both are why this sprint was deliberately sequenced after a week of real data:

- **`no_data` may be a normal terminal state, not an anomaly.** Six parcels went dark
  *simultaneously*, all last seen "1.7d ago", all with a last status of "Відправлення прямує до X".
  If Nova Poshta simply stops resolving an export once the US carrier takes over, then `no_data`
  will accumulate every week, the attention group will bloat, and — worse — **a parcel that actually
  arrived will sit in "needs attention" forever**, because nothing will ever move it to delivered. A
  false alarm that never clears is worse than no signal. See `WB-TRACK-NO-DATA-TERMINAL`.
- **Many "overdue" parcels have already arrived.** Reading their statuses: "Відправлення у Pensacola.
  Очікуйте повідомлення про прибуття", Chandler, Tallahassee, Palm Springs. The parcel is at
  destination waiting for the recipient to collect it. That is still actionable, but it is a
  different action — nudge the customer, not hunt the parcel — and may deserve its own state rather
  than sharing one with "stuck in transit".

**Verified on prod:** attention ordering (code 111 leads, the seven `no_data` follow, then
`59500007044916` at 12.68d overdue); `POST /tracking/refresh` carries its **own**
`require_role(OWNER, MANAGER)` in the deployed image (403 unauthenticated), cooldown enforced
server-side (429 with `Retry-After`, then 200 `{polled:53, changed:8}`, then 429 again); one
`run_poll` defined at `wb_tracking_service.py:422` with exactly two callers,
`routers/westernbid.py:212` and `scheduler.py:295`; MCP back-compat unchanged. The last open
verification bullet — a row expanding on a parcel with **more than one** event — is now closed on
real prod data: 21 parcels carry two events, e.g. `59500007088348` code 6 → code 9 and
`59500007090113` code 101 → code 9, oldest-first with dates. Visual confirmation of the rendered
page was done by the planning agent in the browser, since CC has no browser tool and prod sits
behind Cloudflare Access.

**Follow-ups filed:** `WB-TRACK-NO-DATA-TERMINAL`, `WB-TRACK-POLL-FLOOR`.

### REFUND-SYNC-STARTUP + PROD-LOGS-EPHEMERAL — two small fixes (closed 2026-08-06)

**Merged fast-forward `086e713` → `313c06e`, deployed.** Backend rebuilt only (no frontend files
changed); alembic head `c7e1b4d93f28` unmoved — and that is *proof* rather than assumption, since
the container command runs `alembic upgrade head` on every start. Rollback images tagged by ID as
`rollback-086e713`.

**The defect, and how it was found.** `run_shopify_refund_sync` was registered as
`interval, days=1` with **no `next_run_time`** — the identical defect fixed for the tracking poll in
`WB-TRACK-2`. Two instances of one defect in two days is a pattern, not a slip. It had **never run
in production**: `order_refunds` holds 3 rows, all `created_at = 2026-07-27`, i.e. all from the
one-off `SHOPIFY-REFUNDS` retro-fix, with zero scheduler-written rows in the ten days since.

**Sized before fixing, and the planning brief was corrected downward.** The brief called this a
money bug because refunds feed the partner-settlement base. By *mechanism* that is right; by
*volume* it is not — Shopify analytics shows returns of −29.99 (2026-05) and −29.99 (2026-07),
nothing else in 120 days, and no refunds at all among the 50 most recently updated orders. The dry
run confirmed it: `found=1 already_present=1 inserted=0`. **Nothing was missing.** Fixed because it
is one line and the mechanism is real, not because data was bleeding. The post-deploy startup run
reproduced the prediction exactly.

**A $8.00 discrepancy, resolved as a definition rather than a gap.** Stored refund amounts
(32.99 / 37.99 / 37.99) exceed the analytics figures (29.99 / 29.99) by $8.00 in both overlapping
months: `totalRefundedSet.shopMoney` includes refunded **shipping**, the analytics `returns` column
does not. Refund counts reconcile exactly. The brief had deliberately left this branch open —
"if the numbers diverge materially, stop and report rather than tuning" — and it landed on the
benign side.

**A startup-policy guard, shaped as a completeness guard rather than a blanket rule.** "Every daily
job must fire at startup" would be too strong — a future daily job may legitimately not want a
startup run, and a flat assertion would simply block it. So the test follows the
`test_money_field_completeness` / `test_route_scope_completeness` idiom: every registered job needs
an explicit verdict (`startup:<reason>` or `deferred:<reason>`), and the test fails on an
unclassified job, a stale entry, a disagreement with the code, or an empty reason. It forces a
verdict, not a particular verdict. Verified to bite: removing the new kwarg fails the test naming
the offending job.

**Verification in the deployed image, by substance rather than ceremony.** The guard test cannot
*run* on prod — the image carries production dependencies only, no pytest. Rather than record that
as done, CC parsed `STARTUP_POLICY` out of the shipped test file with `ast` (side-stepping the
pytest import) and cross-checked every entry against what `start_scheduler` actually registers in
that image: all five agree — `run_shopify_sync` and `run_westernbid_poll` deferred with no
`next_run_time`, the three daily jobs set. The job then fired **2 ms after registration** instead of
24 hours later.

**`PROD-LOGS-EPHEMERAL` closed in the same window** — a bind mount for `/app/logs` to
`/home/prorder/orderhub-logs`, deliberately outside the git clone, applied with `up -d backend` and
no rebuild, with the usual timestamped `.bak` of `docker-compose.prod.yml` first. Verified by
force-recreating a third time: the file went 30 → 61 lines and now holds **both container
generations**, two refund-sync fires in one file. That capability did not exist this morning. It had
cost three times: twice the log survived only because someone copied it by hand, and the third time
it was *structurally unable* to answer "when did this job last run" — which is why the answer had to
come from row provenance instead.

> **Carry this forward:** `docker compose logs` is **still** ephemeral and no volume can fix it —
> container stdout begins at container start, by Docker's design. `backend/logs/server.log` is now
> the durable record; the docker log is not. AI_ONBOARDING §9 already says to grep the file rather
> than docker stdout, for a different reason; this is the second, stronger one.

**Found while sizing, filed not fixed:** `PARTNER-REFUND-BASE` — the partner settlement base uses
gross `items_revenue` with no refund deduction anywhere, so a partner is paid on refunded revenue.
That is the real version of the concern the brief had misplaced onto the scheduler: business logic,
not job cadence.

---

### PARTNER-CONFIG-1 — partner entity + per-shop percent, basis and settlement currency (closed 2026-08-07)

**Status: MERGED fast-forward `765c298` → `152ad1c` and DEPLOYED to prod 2026-08-07** (CC executed
the full chain: merge → 913/215 green on `main` → push → server pull → rebuild backend+frontend →
migration `d7f3a1c85e92` at head; deploy addendum at the end of this entry). The full design
record — requirement, decisions ①–④, review amendments ⑤–⑫ — stays in the PARTNER-CONFIG-1 row
below, marked CLOSED but retained.

**What shipped.** Global `partners` entity + per-shop `shop_partner_config` (percent, basis,
settlement currency); Partners tab in Edit Store Settings; entity-fed partner picker with config
prefill and per-settlement percent override. Two new PG-enum bases — **TURNOVER** (items revenue −
funded discounts − refunds dated in the period; before fees, shipping fully excluded) and **PROFIT**
(turnover − COGS − non-shipping fees − FX-converted allocated overhead) — both net of
`discount_total` (confirmed with Sergii in plan mode); refunds consumed via Model 2 dating. Legacy
formulas still deserialise, are not selectable for new configs. `fx_rate_used` frozen onto
settlements; conversion via `fx_service.resolve()`; loud 422 when no usable rate. **Hard overlap
block** per (shop, partner) server-side (`_overlap_predicate`, closed-interval — a single shared
day counts) mirrored client-side by a red preview panel + disabled Save; UI defaults the next
period to `last period_end + 1`. Negative bases pass through as negative settlements. Preview
ships the **base-quality panel** (missing-cost count, unimported closed Etsy months, Shopify
NULL-fee count, Etsy-refunds-unbooked note) and unpaid settlements get a staleness recompute
("Check for changes") using stored rates. `partner_config_audit` is a purpose-built table —
deliberately NOT `AccessAudit`, whose `target_user_id` means "whose access changed"; a partner is
not a user. Migration backfills `partners` from DISTINCT names across both tables with a
provably-total `partner_id` backfill; list filters moved `partner_name` → `partner_id` (filtering
on the snapshot string would recreate the split-balance bug the entity exists to kill). Fee
backfill reuses the existing OWNER-only endpoint; cross-shop `GET /api/partners/balances` is
OWNER-only; `PartnerNameInput.tsx` deleted (orphaned), backend `/partner-names` retained as a
cleanup candidate.

**Verification.** Backend 913 nodes green (755 → 780 `def test_`); vitest 215 in 35 files (was
187/34); `tsc -p tsconfig.app.json` 27 errors **byte-identical to `main`** (drift 21 → 27 noted in
TYPECHECK-1); lint 88 identical; migration round-trip clean with a **tested downgrade guard** —
with a turnover settlement present, downgrade raises RuntimeError and leaves the DB at head (PG
enum values are un-droppable; the guard makes irreversibility loud). PG16 permits `ADD VALUE`
inside alembic's transaction; it forbids *using* the value there — hence no `server_default`/CHECK
on `shop_partner_config.basis`, plus a standing docstring rule that no future migration may write
the two new values.

**The money guard caught a real leak mid-build** (third catch to date): the PROFIT preview's term
breakdown exposed period COGS on a `view_finance`-only route; fixed with `_strip_cost_terms`
mirroring `routers/finance.py:_strip_itemised_costs`, route classified
`view_finance,view_costs-strip`. The two completeness guards also caught three unclassified fields
and four unclassified routes during the build.

**Smoke (Cowork browser MCP, dev, 2026-08-07): 10/10 PASS + a focused FX retest PASS.** PROFIT
preview with a 1 000-UAH June overhead receipt: `allocated overhead (UAH) −22.34 USD` at NBU
44.7626, base 59.98 → 37.64, share 3.76 — arithmetic exact (22.34 × 44.7626 = 999.99), preview
text "Converted at 44.7626 UAH/USD — this rate is frozen onto the settlement". Overlap attempt →
red panel + disabled Save. Both runs: zero console errors; cleanup complete except the overhead
receipt (no UI delete exists — `OVERHEAD-RECEIPT-DELETE`). Smoke follow-ups filed:
`PARTNER-CONFIG-1-followup-1/2/3`.

**Unverified, carried to the runbook:** OQ2 against prod (CC's read-only ssh+psql was blocked by
the permission classifier — the §9 mid-flow approval quirk, **not a capability boundary**: CC
deploys and reads prod over `ssh orderhub` as standard, AI_ONBOARDING §4.1 clarified 2026-08-07;
the schema answer is conclusive — payments survive deletion with
`settlement_id = NULL`, no cascade — but payments are **immutable**, so any linked payment cannot
be relinked: delete + recreate it too, or accept a progress-badge gap). The FX no-rate 422 path is
unit-tested only. OWNER-only gating was not exercised with a manager login. The spec's original
sanity check was unsatisfiable as written (planning-agent error: Finance has no items-revenue
card); CC restated it in the plan file — the SQL cross-check stays on Sergii's list.

**Post-deploy runbook (corrected 2026-08-07: CC executes ①④⑥ from the dev CLI incl. `ssh orderhub`; Sergii does the UI/business steps ②③⑤; order matters):** ① CC: merge → push → server `git pull` + rebuild backend & frontend + `docker compose -f docker-compose.prod.yml exec backend alembic upgrade head` (SERVER_DEPLOY_PLAN §5–§7 conventions; no `.env`/overlay change expected this deploy);
② set Platform Fee for Lamamarka Shopify (recommended **7.5%** — first verify one real WB
settlement against the PayPal-US tier and Shopify Billing's 1%, amendment ⑪); ③ run the fee
backfill endpoint; ④ run the Kseniia payments SQL from CC's plan file **before** ⑤ deleting her
settlement; ⑤ Partners tab → configure Kseniia (the entity already exists — the migration created
it; a config is deliberately not auto-created because a migration cannot invent percent/basis) →
recreate the settlement under the agreed basis; ⑥ dev residue: SQL-delete the 1 000-UAH smoke
receipt (archived overhead item "FX Smoke Ad", `b4fcfa2d-46e3-4987-91a7-14693e45cdb8`) and
optionally the `Smoke Partner A/B` / `FX Smoke Partner` entity rows.

**Closes:** `PARTNER-REFUND-BASE`, `PARTNER-SETTLEMENT-STALE` (a → panel, b → staleness recompute)
— rows marked CLOSED below. `STATEMENT-MONTHLY-GUARD`'s settlement-time half is covered by the
panel; its dashboard half remains open (row updated). Filed: `PARTNER-CONFIG-1-followup-1/2/3`,
`OVERHEAD-RECEIPT-DELETE`. Docs commit:
`docs(partner-config-1): close sprint with entity+config partner model, FX-aware bases`.

**Deploy addendum (2026-08-07, CC).** No `.env`/overlay change was needed — verified by sweep (the
39 files touch only backend app code + frontend/src; zero Dockerfile/compose/env/config matches,
zero new env reads). Prod migration was applied by the container's own start command; the explicit
`exec … alembic upgrade head` confirmed head as a no-op. Startup log clean (5 scheduler jobs, FX
refresh OK at 44.7626, only the expected idlaser-missing warning). **Route-gating check restated by
CC, correctly:** the spec's "403 at the tunnel hostname" is not observable because Cloudflare
Access fronts `/api/*` — the edge answers 302 to the Access login for unauthenticated requests
(routes not publicly reachable at all); the authoritative in-container check returned 403 on all
three new surfaces (`/api/partners/balances`, `/api/partners`, `/api/shops/{id}/partner-config`).
**OQ2 resolved cleaner than feared:** the Kseniia settlement has **zero** linked payments — in fact
the payments table is empty — so delete-and-recreate is unobstructed and the immutable-payment
caveat is moot; her partner entity exists from the migration backfill (`bacaf4ee-…`). Dev residue:
smoke receipt + "FX Smoke Ad" material deleted (1 row each, 0 remaining); the three smoke partner
entities were retained because each carries 2 `partner_config_audit` rows — the audit trail
correctly outlives the config, and deleting the entity would orphan it (accepted residue, covered
by followup-3's entity-lingering note). TURNOVER sanity: SQL mirror of
`finance_service.compute_turnover_terms` returned **59.98 exactly** (items 59.98 − discounts 0 −
refunds 0), matching the smoke preview. **Still open after deploy:** Sergii's ② fee rate → ③
backfill → ⑤ Kseniia config + resettle; OWNER-vs-MANAGER gating with a real manager token remains
unexercised. **Correction (2026-08-07, surfaced by the ③ dry run):** Kseniia's settlement is on
**Lamamarka ETSY**, not Lamamarka Shopify — the planning agent had assumed Shopify. The Shopify
fee backfill therefore has **zero** interaction with her settlement (`overlapping_settlements`
came back empty, shop-scoped, cause verified by CC). ⑤'s real dependencies: the Etsy statement
import for her period (already done, 2026-08-06) — and the `ETSY-REFUNDS` caveat: Etsy refunds
are booked nowhere, so her recreated base's refund deduction is **inert** and overpays by roughly
(her period's Etsy refunds × her percent); the base-quality panel surfaces this. **Decided 2026-08-07: ⑤ is DEFERRED until
`ETSY-REFUNDS` ships** (reminder scheduled for Mon 2026-08-10 — start the ETSY-REFUNDS spec, then
resettle Kseniia). ETSY-REFUNDS is already sized (see its row) and is the natural next sprint. **② done 2026-08-07** (7.5% set in prod UI).
**③ DONE 2026-08-07 (live run after dry-run approval):** updated 447 == matched 447; post-write
NULL-fee non-CANCELLED count 0; parity with the import path **by construction** (the backfill
calls `compute_platform_fee`, `order_service.py:82-111` — base is `Order.total_price`, the full
customer charge incl. shipping+tax, ROUND_HALF_UP once), exhaustive mismatch check 0; the 39th
NEW order already carried a fee (imported after ② — incidental proof the rate is live at import);
test order `91890_1559` priced 3.34, inert at status NEW (`SHOPIFY-REFUNDS-followup-1` still
parked); containers healthy, durable log clean. Net effect: **$1 543.37** of fees entered the
Jan–Jul Shopify P&L (409 orders), $137.00 pending on 38 NEW orders. Docs committed + pushed
`d8fa10e` (incl. `.gitignore`-ing `start-dev.sh` — permanent end of its git-status noise).
...
WH-1 (this) → WH-2 (consumption at SHIPPED + cost into COGS + counter switch) → WH-3
(opening balance + agent-driven purchase-history entry) → WH-4 (MCP packaging tools).

**What shipped (CC, Opus, plan-mode gated):**
- Migration `e9b2f4c7a318` (off `d7f3a1c85e92`): `materials.category` (plain VARCHAR
  `MATERIAL|PACKAGING`, Pydantic-validated — deliberately NOT a PG enum, per the
  `Capability` precedent and the PG16 `ALTER TYPE` trap), `materials.is_stock_tracked`
  (Boolean, default true), `packaging_boxes.material_id` (NOT NULL UNIQUE FK, RESTRICT),
  one material backfilled per existing box (rule-4 frozen values: unit `шт`, UAH, cost 0,
  stock 0 — counters stay authoritative on `packaging_boxes` until WH-2).
- Paired lifecycle in `catalog_service`: create box → material+geometry in one
  transaction; box rename syncs material name; box delete **archives** the material.
  `PATCH /api/materials` answers **409** for rename/category change on a box-backed
  material (packaging surface is the single entry point for those).
- Consumption: `is_stock_tracked=false` BOM line prices into the currency buckets as
  before but stages no movement, moves no counter, raises no negative-stock warning.
  FX all-or-nothing rule untouched. **Closes `SVC-MATERIAL-NONSTOCK`** (flag flip on the
  real cutting/sewing materials = post-deploy operator action, still pending).
- Frontend: BOM picker filtered to `category=MATERIAL`; "Show packaging" toggle on
  materials page; visible NO STOCK badge + banner on untracked lines ("contributes cost,
  does not consume stock" — the ERPNext silent-skip lesson); checkbox on create form.

**Deviations CC flagged (reviewed, accepted):**
- **Idempotency guard widened**: `consume_materials_for_order` now also skips when
  `order.computed_production_cost` is already set — needed because an all-untracked
  recipe leaves no ledger row to block a re-ship. Strictly narrowing vs the movement
  probe; one accepted delta pinned by a test: an order whose first ship failed FX
  (cost=None) still recomputes on a later ship.
- Low-stock badges suppressed for untracked materials; receipts/adjustments deliberately
  NOT disabled for them (an untracked service material needs a qty-1 receipt to carry its
  unit cost — existing BOM-costing convention).

**Verification (CC):** backend 913 → **947** green incl. both completeness guards
(str/bool/UUID invisible to the money guard's float/Decimal filter; no new `{shop_id}`
routes); frontend 215 → **225** vitest green; tsc + eslint **byte-identical to `main`**
(compared via a real worktree); migration round-trip on dev (3 boxes): upgrade →
downgrade → upgrade re-linked the same ids, operator flag survived; live service-level
smoke against dev Postgres (create/rename/delete-archives).

**Cowork browser smoke (2026-08-07, dev, Opus subagent): 10/10 PASS, zero 4xx/5xx across
39 `/api/` calls, console clean.** Verified end-to-end: packaging page regression; box
create → paired PACKAGING material (default materials call is `?category=MATERIAL`,
toggle drops the filter); untracked material shows NO STOCK badge and no low-stock badge;
BOM picker excludes packaging, includes the untracked service material, banner renders;
rename sync; delete → archived (visible under "Show archived", not hard-deleted);
**backfill confirmed incidentally** — the pre-existing box `100x120x50` carries its
paired material. Two archived SMOKE-* material rows remain in dev (harmless; DB-level
delete if a pristine dataset is ever wanted).

**Follow-ups filed (see Explicitly deferred):** `WH-1-followup-1` (unit vocabulary:
material form offers `dm2/m2/pcs/m/kg` but packaging-paired materials carry `шт` — a
manually created piece-material can't match the packaging unit), `WH-1-followup-2`
(packaging delete fires with NO confirm dialog while the less-destructive material
archive HAS one — bundle with WH-2, which reworks the delete path anyway).
...
### WB-ALERTS-1 — parcel alerts on the dashboard + usage note on /westernbid (closed 2026-08-20)

**Merged fast-forward, `main` = `ad2f441` (incl. docs commit `088c168`), deployed to prod
2026-08-20.** Migration `a3c5e7b91d40` → `b6e2d4a91c73` (single head), applied by the entrypoint.
Rollback images tagged by commit: `orderhub-backend:rollback-f5bb75f` (`5a842998059d`) /
`orderhub-frontend:rollback-f5bb75f` (`008dcbaeb6e7`); rollback order is `alembic downgrade -1`
(clean DROP TABLE — only alert data lost) **before** reverting images.

**Why it exists.** The 2026-08-20 review (`docs/reviews/2026-08-20-wb-track-no-data-checkpoint.md`)
surfaced two findings: `no_data` is a self-clearing transient (closing
`WB-TRACK-NO-DATA-TERMINAL`), and — the operationally decisive one — **nobody had opened
`/westernbid` even once in two weeks**. The classification is trustworthy; its findings reached no
one. Telegram was considered and rejected by Sergii (messages drown); the decision was push onto
the surface people already open daily — the dashboard — with alerts that clear either
**automatically when the condition disappears** or **manually with a who+when record**.

**What shipped.** Table `wb_parcel_alert`, keyed on `wb_parcel.shipment_id` so untracked UPS/USPS
parcels (no tracking row) are alertable; a **partial unique index** on `(shipment_id, kind)
WHERE resolved_at IS NULL` enforces one-open-alert-per-parcel-per-kind in the database rather than
in code. `detect_alert_conditions` is pure over `ClassifiedParcel` — nothing is re-classified;
`sync_alerts` runs inside `run_poll` on both caller paths (scheduler + manual refresh), never
commits, and is deliberately not wrapped in try/except. Two endpoints (list + dismiss), both
OWNER/MANAGER; display fields are joined live from the mirror, not snapshotted. Frontend: a
collapsible block above the dashboard-query gate (ignores the period selector; quiet all-clear
line with sync time when empty; untracked rows fall back to carrier numbers), and the collapsible
Ukrainian "Як користуватись" note on `/westernbid`. Thresholds (6d dark / 7d overdue / 14d
untracked) are app_settings-overridable per the `DEFAULT_STALLED_DAYS` pattern.

**Dismissal semantics worth recording:** a dismissed alert's row **stays open in the DB**
(`resolved_at` NULL, so the partial index keeps blocking a duplicate while the condition
persists) but leaves the UI; when the condition later clears, it resolves as cleared **with the
dismissal record intact**; a cleared-then-recurring condition is a new episode and re-raises.
Aged-out parcels resolve their alerts as `aged_out`.

**Two spec deviations, both approved before building:** `failed_delivery` became
`delivery_problem` keyed off the whole `problem` state (all 14 codes, not just 111 — a code-102
return needs the alert no less), and one parcel may hold several kinds simultaneously.

**A defect worth remembering, found and fixed in-sprint:** the scheduler change read the new
summary counters directly, so a summary lacking them raised `KeyError` *after* the commit, inside
the job's own exception handler — **a successful poll reported as a failure** with a pointless
rollback. The existing test drove that exact path and passed anyway, because it asserted only that
commit was awaited; it now also asserts rollback was **not** — verified red by reintroducing the
bug. Same blind-instrument family as the WB-TRACK-2 scheduler find; third instance in the
scheduler in two weeks.

**Verification.** Backend 1033 → 1076 tests; both completeness guards green; migration round-trip
clean; partial index + named FKs confirmed in `\d`. Frontend 252 → 270 tests; tsc 21 and lint 87
diffed **byte-identical to `main`** in a worktree rather than compared by count (the recorded lint
baseline of 88 was already stale on `main` — correction folded into `TYPECHECK-1`'s numbers).
Dev smoke on real data: dismiss records who+when; re-sync opens nothing; clearing resolves-as-
cleared keeping the dismissal; re-occurrence opens a new episode; >60d resolves `aged_out`; 403
unauth / 403 DESIGNER / 200 OWNER; `age_days` a JSON number. **Cowork browser smoke on dev and
prod:** block renders above dashboard content, collapses/expands, dismiss removes the row with a
toast and decrements the badge live; help note collapsed by default with the full appendix text.

**The launch wave was the strongest possible validation.** The first prod poll generated **16
alerts: 8 `overdue_long`** (the long tail of ~17 overdue — selective, not exhaustive),
**7 `untracked_aging`** (the known UPS/USPS backlog), and **1 `no_data_stuck` —
`59500007135457`**, the exact parcel the 2026-08-20 review had singled out by hand. The mechanism
selected it independently from zero. It is also the live dual-kind case (`no_data_stuck` +
`overdue_long`, adjacent rows). Meanwhile two freshly-dark parcels (1.2d) correctly did **not**
alert — the 6-day threshold doing its job on day one. The 16 are real; dismissing them is the
operator's first use of the feature, deliberately left to Sergii.

**Follow-on process:** a weekly Cowork parcel-health review is scheduled (Mon 10:00), replacing
the one-off checkpoint; it also watches whether alerts are actually being dismissed — if nobody
dismisses, the dashboard surface is failing the same way `/westernbid` did, and that is a finding,
not a silence.

**First-week outcome (2026-08-24 weekly, `docs/reviews/weekly-parcel-health-2026-08-24.md`).**
Open alerts **16 → 23** (14 `overdue_long`, 7 `untracked_aging`, 1 `no_data_stuck`, 1
`delivery_problem`); 24 raised, **1 auto-resolved as `cleared`**, **0 dismissed**. The zero is
**not** the `/westernbid` failure repeating: Sergii confirmed 2026-08-25 that **day-to-day order
handling has not migrated into OrderHub yet** — the CRM runs as a mirror, so nobody opens the
dashboard as part of their work. Treat 23/1/0 as the **pre-migration baseline**, and do not
re-litigate dismissal behaviour until the first week after migration. **The alerts were vindicated
on substance instead:** both parcels they singled out were investigated by hand on 2026-08-25 and
**both were real** — `59500007135457` physically stuck 16 days after "plane arrived at New York"
(vs ~2 days NY→US-carrier on a sibling parcel; WesternBid inquiry opened), and `59500007140095` a
genuine undeliverable-address return. Zero false positives in the first week.
**Two defects surfaced by that investigation, both filed:** `WB-TRACK-EVENT-COMPLETENESS`
(our mirror holds code 120 while NP's own site shows two later events the same day) and
`WB-CODE-102-LABEL` (code 102 renders as "Відмова від отримання" on a parcel the recipient never
saw). Also filed: `WB-ALERTS-2` — `untracked_aging` (7 alerts, 30% of the open set) has **no
self-clearing path by construction**, so it can only leave via dismissal or age-out.
**Schema note for future queries:** the timestamp column is `raised_at`, not `created_at`.

### WB-POLL-CADENCE — closure confirmation (2026-08-26)

The full record lives in this table's `WB-POLL-CADENCE` row (filed, built `42002cb`, deployed
2026-08-25 — including the self-confirming-measurement lesson). What was still owed at deploy
time was the **formal two-poll-gap evidence**, and it is now in hand: prod poll instants
**08-25 16:55:37 (boot) → 08-25 20:55:37 → 08-26 08:55:37**, all on the same `:55:37` grid, gaps
of exactly 4h and 12h (= 3 × 4h) — a pattern a daily interval cannot produce. **Closed.** The
~2-week data-accumulation clock for `WB-TRACK-1-followup-1`, the stub flicker-vs-outage question
and the floor re-measurements (`WB-TRACK-CODE-80`, no_data ceiling) started 2026-08-25.

### CASE-1 — per-order case tracker: issues, returns, reships, reviews (closed 2026-08-26)

**Merged fast-forward, `main` = `e7a93e1` (`b23beb1..e7a93e1`, 22 files, +3304/−1), deployed to
prod 2026-08-26.** Migration `b6e2d4a91c73 → c8f4a2e70b19`, single head, applied by the
entrypoint (verified in container stdout, not inferred). Rollback images
`orderhub-{backend,frontend}:rollback-b23beb1` (`4236d08c5cdc` / `4d0828022225`); **rollback
order is counter-intuitive and recorded here deliberately: `alembic downgrade -1` BEFORE
swapping images** — the entrypoint runs `upgrade head` on every start and the old image has no
`c8f4a2e70b19` revision file, so an old container against the new schema crash-loops under
`restart: unless-stopped`.

**Why it exists.** Sergii's core need — surfaced by correcting a Cowork draft that had drifted
toward more parcel automation — was manager workflow for problem orders: a return, a 1★ review,
a reship offer, "waiting on the customer's address" all lived in Gmail and in heads. Design
settled conversationally and recorded in `docs/design/order-cases-plan.md`; the parcel half of
the original uploaded spec is parked as its own track
(`docs/design/parcel-watchlist-research-2026-08-26.md`). The Promise/Claim entities from that
spec deliberately dissolved into case fields (`due_at` + `next_action`) and a case type.

**What shipped.** `order_case` (type, title, status `in_progress|waiting|resolved`,
`next_action`, `due_at`, owner, `resolution_note`) + `order_case_note` (append-only, the
`order_status_history` pattern, with a `kind` column separating `system` from `comment`).
Status transitions write system notes with the acting user; a non-empty resolution summary is
appended to the closing system note AND stored in the column. Order card: `DetailCases` in
`OrderDetailView` behind `(isOwner || isManager)`. Dashboard: "Питання по замовленнях"
collapsible block (WB-alerts pattern) — groups **В роботі** / **Чекаємо**, overdue-first with
`Прострочено N дн. · до <date>` in the WB block's vocabulary, count badge, quiet all-clear line,
row click → order card. Notes on resolved cases stay allowed — **deliberate**: an append-only
timeline that can grow after closure is a feature ("клієнт подякував"), not a leak.

**Decisions worth preserving (all argued before build, two against the task.md's literal text):**

- **Strings + Python enums, not PG enum types.** CC surfaced that the freshest precedent
  (WB-ALERTS-1) deliberately dropped PG enums; `case_type` will certainly grow, and with PG
  enums every new value is an `ALTER TYPE` migration. Cowork agreed — rule 1's "enum" reading
  was overridden consciously, matching the newest layer rather than the oldest.
- **`kind` column over `author_id IS NULL`.** The NULL-author encoding would have lost WHO
  changed a status — rule 3 required author+time on transitions. The rejected alternative is
  exactly the `humanizeComment()` regex-parse debt `DetailTimeline` already pays for a column
  saved years ago; not repeated.
- **The router-level role gate is not redundant.** `assert_order_access`
  (`routers/dependencies.py:96-124`) admits a DESIGNER on orders assigned to them — relying on
  it alone would have quietly given the assigned designer read AND write on cases. Pinned by
  `test_case_router_gate_beats_assert_order_access_for_a_designer`.
- **`/cases/open` is shop-scoped, unlike the WB alerts endpoint it visually mirrors.** Parcel
  alerts are global (`routers/westernbid.py:262-265`); cases filter through `get_shop_scope`,
  with a leak test (`test_open_cases_excludes_unscoped_shops`). Copying the neighbour's access
  model because the UI looks the same would have been wrong silently.
- **Honest scope-test extension.** `test_route_scope_completeness`'s scanner sees only routes
  with `{shop_id}` in the path — the cases routes would have passed **by invisibility**. All
  five were added to `INDIRECT_SHOP_ROUTES` with described enforcement + behavioural tests.

**The blocking defect the browser smoke caught — and the two lessons in it.** The first build
(`54941b5`) wired `DetailCases` into `OrderDetailPanel.tsx` — a component with **zero importers
since `15d7b5d`**; the live order card is `OrderDetailView.tsx` (`pages/OrderDetailPage.tsx:28`).
Every one of the 14 component tests passed while the section rendered for nobody, because
nothing asserted composition. Lesson 1: **CLAUDE.md's gotcha still named `OrderDetailPanel` as
the refactored card** — stale docs steered the wiring; the gotcha is corrected in the same docs
commit as this entry. Lesson 2: composition needs its own pin — the fix round added
`OrderDetailView.test.tsx`, which fails 2-of-3 on the pre-fix code. Fix round `e7a93e1` also
fixed: **stale PATCH timeline** (the status-change system note was written via bare FK, so no
`back_populates` event fired and the post-commit re-query hit the identity map where
`selectinload` won't overwrite a populated collection — fixed at the write via a shared
`_build_note(case=, author=)`; the same latent bug existed in `add_note`); dashboard overdue
figure; `window.prompt` → inline themed summary input; summary → closing system note.

**Two pre-existing defects found while building, both filed rather than fixed (out of scope):**
`order_status_history.changed_at` has `server_default="now()"` as a plain string — Postgres
froze it at DDL time, so **every default-stamped audit row since April carries the same fake
timestamp** (verified on the live DB; CASE-1 itself uses `func.now()` / `sa.text("now()")`
everywhere); and `routers/orders.py:190` serves the identical stale-response shape for
`status_history` on PATCH — the idiom CASE-1's router had copied. Filed as `OSH-TIMESTAMP-1`.

**Verification.** Backend 1076 → 1103 (nothing lost across both rounds); vitest 270 → 303; tsc
21 / lint 87 — byte-level baselines held through both commits; migration round-trip clean.
**Dev browser smoke (Cowork, Opus subagent): two rounds** — round 1 found the blocking wiring
defect plus D2–D5 (its dashboard-side results: grouping, overdue-first red sort, badge,
collapse, all-clear, row navigation, WB-block coexistence all PASS); round 2 after `e7a93e1`:
**9/9 PASS, everything driven through the UI**, including immediate system-note render on
status change (no reload), `Прострочено 6.3 дн.` matching the due date to 0.02d, inline summary
box, resolved-stays-in-card after reload, console clean. **Prod:** deploy verified by CC (403
unauthenticated / 200 + `{"in_progress":[],"waiting":[]}` with OWNER token — the 200 also
proves the tables exist; tunnel healthy); **Cowork prod render-smoke 5/5 PASS** (read-only,
2026-08-26): dashboard all-clear line `Питання по замовленнях: немає відкритих` between the WB
block (intact, badge 21) and the KPI row; `Питання` card with `+ ПИТАННЯ` renders on a live
order between Product inventory and Notes from customer — i.e. wired into `OrderDetailView`,
not the dead panel; all neighbouring sections render; console clean (only the pre-existing
recharts warnings). One cosmetic observation, not filed: the zero-state is a bare muted line vs
the WB block's full card — fine as-is, revisit only if the first real prod case renders oddly.

**Follow-ups filed:** `OSH-TIMESTAMP-1`, `ORDER-SEARCH-1` (smoke by-catch: orders search cannot
find an order by its own number — pre-existing, unrelated to cases), `CASE-1-followup-1` (UX
bundle), `DEPLOY-RUNBOOK-1` (CC: `SERVER_DEPLOY_PLAN.md` has no recurring-deploy runbook and its
§4 compose YAML is stale four ways — followed live server state instead).
...
| PARTNER-REFUND-BASE | **CLOSED 2026-08-07 by PARTNER-CONFIG-1** — both new bases deduct refunds dated in the period; legacy formulas stay uncorrected by design (read-only). ~~A partner is paid on revenue that was refunded to the customer~~ | Found by CC 2026-08-06 while sizing the refund-sync defect, and correctly handed to the planning agent rather than fixed in passing. `compute_revenue_items_minus_fees` (`finance_service.py:380-395`) computes `items_revenue − non_shipping_fees` with **no refund term anywhere**, so a partially- or fully-refunded order still counts in full toward the partner's settlement base. This is the real version of the concern the brief had misplaced onto the scheduler: **a business-logic gap, not a job-cadence bug**. It is not theoretical — the first Etsy statement import measured **−338.12 of refunds, 177.56 of it on orders live in the P&L**, plus Shopify refunds on top. **Sequencing luck:** the one existing settlement (Kseniia, 1 030.65, 2026-01-01…07-31) is **UNPAID**, so nothing has actually left the business yet and the base can be corrected before it does — verify that before doing anything else, because a paid settlement is closed history and must not be recomputed (`PARTNER-SETTLEMENT-STALE`). Note the interaction with `SHOPIFY-REFUNDS`: Model 2 already books refunds as dated `order_refunds` rows netted in the refund's month, and the partner formula simply does not consume them — so the data mostly exists, the formula does not read it. Etsy refunds are a different matter and are still unbooked entirely (`ETSY-REFUNDS`), which means a full fix depends on that too. Fold into `PARTNER-CONFIG-1` — a formula that ignores refunds is a formula problem, and that sprint is already redesigning the formula layer. |
| PARTNER-CONFIG-1 | Partners need real configuration — per-partner percentage AND per-partner basis | **CLOSED 2026-08-07 — built on `feat/partner-config-1` (152ad1c); NOT merged, NOT deployed. See the closure entry above. Row retained as the durable design record.** **DESIGN AGREED 2026-08-06 — ready for a spec. Read this row in full before writing `task.md`; it is the handoff.** <br><br>**Requirement (Sergii):** several partners, and the number will grow. Each carries his own percentage **and his own basis** — with one partner he settles on turnover, with all the others on profit. Partners are configured **in shop settings**: add a partner to a shop, set his percent, set which kind of percent. <br><br>**Decisions taken, do not relitigate:** ① **"Turnover" = all revenue excluding shipping, BEFORE platform fees** — the gross item price. This is a **third basis that does not exist today** and applies to exactly **one** partner; Sergii called it a deliberately unique option, so build it as a first-class basis but expect a single user. ② **"Profit" = revenue − COGS − fees − shipping − allocated overhead**, i.e. **overhead included** (advertising and the rest). This matches what `finance_service` already computes for the Finance page and therefore needs no new aggregate; note it also picks a side in `NETPROFIT-RECONCILE`, where the dashboard omits overhead and Finance subtracts it. ③ **Several partners per shop**, each independent — different percent, different basis, computed separately against the same period. ④ **Refunds reduce the base of the period in which the REFUND happened**, never by re-opening a past settlement. This is elegant and free: `SHOPIFY-REFUNDS` Model 2 already books refunds as dated rows in the refund's month, so the data is already shaped correctly and settlement immutability is preserved. <br><br>**What exists today:** `partner_name` is a free `String(200)` typed in at settlement time on `PartnerSettlement` and `PartnerPayment` — no entity, no list, so a typo silently splits a partner's balance. Percent is entered per settlement, not configured. Two bases only (`PartnerSettlementFormula`): `REVENUE_ITEMS_MINUS_FEES` and `NET_PROFIT_PRODUCT_ONLY`. Settlements and payments are **immutable post-create** (`partner_payout_service.py:9`) — keep that; a settlement is a historical fact. <br><br>**Three dependencies, and the first can invalidate the whole profit basis:** ① **COGS coverage.** The profit basis is only as good as recorded cost, and costing is **forward-only** — `FX-CONVERSION` fixed new shipments, already-shipped orders keep NULL cost (`historical-COGS-recompute`, still open), and BOMs exist for very few products (the pilot covered one wallet). If most orders carry no cost, "profit" ≈ revenue minus fees, overhead and shipping — **overstated, and the partner is overpaid**. **Measure this before building: what share of the relevant shop's orders in a realistic period actually carry a cost?** If it is low, sequence COGS first. ② **`ETSY-REFUNDS`.** The refund deduction works on Shopify (`order_refunds` is populated) but **not on Etsy**, where refunds are parsed, counted and reported but booked nowhere — measured at −338.12 across 2026-01…07. An Etsy-shop partner cannot have refunds deducted until that ships. ③ **`PARTNER-REFUND-BASE`** is the bug this sprint fixes; see its row. <br><br>**Open questions for the spec:** does `REVENUE_ITEMS_MINUS_FEES` remain selectable for new partners, or become legacy-readable only (the one existing settlement uses it, and immutability means old rows must still deserialise)? May a settlement **override** the partner's default percent/basis — likely yes, since it already snapshots both and rates change mid-relationship? How is **unallocated (NULL-shop) overhead** treated, given a partner's base subtracts *allocated* overhead and some overhead is deliberately global (CLAUDE.md)? Who may create or edit a partner — OWNER only, given it directly determines money out? What happens to the **existing free-text settlement** (Kseniia, 1 030.65, `Items − Fees`, 2026-01-01…07-31) on migration — it is **UNPAID**, so it can be deleted and recomputed once the new bases exist, which is the clean answer, but confirm before doing it. Where in shop settings does the partner UI live (the shop editor already has Platform API and Logistics tabs)? <br><br>**Risk worth stating in the spec:** the two schemes distribute risk oppositely. A turnover partner is paid **whether or not the shop made money** — on Etsy, where the platform takes 32.5% of base, a turnover share plus platform fees plus materials can exceed the margin in a bad month. A profit partner shares the downside. That is a business choice, not a defect, but it should be visible to whoever sets the rate. <br><br>**AMENDMENTS — design review 2026-08-06 (Cowork + Sergii); these supersede the text above where they conflict:** ⑤ **Shipping economics are fully EXCLUDED from every partner basis** (supersedes ②'s formula — Sergii: shipping revenue, shipping cost and anything shipping-adjacent stay out of partner math for now, maybe revisited later). Profit basis = `items_revenue − COGS − non-shipping fees − allocated overhead − refunds`. This **preserves** `docs/design/profit-definition.md` §6 rather than reversing it: the new basis is exactly `NET_PROFIT_PRODUCT_ONLY` + a refund term + FX-converted overhead. It does NOT equal the Finance-page net profit (which nets shipping economics), so `NETPROFIT-RECONCILE` is **not** settled by this sprint. Implement as a **new** `PartnerSettlementFormula` value; never redefine the two existing ones — legacy rows must keep their meaning. (Correction while reviewing: **both** existing formulas lack the refund term, not just the one `PARTNER-REFUND-BASE` cites — `NET_PROFIT_PRODUCT_ONLY` also pays on refunded revenue.) ⑥ **FX handling** (the currency wall): overhead is bought in UAH, all prod shops settle in USD, and `finance_service` is FX-blind — today UAH overhead never reduces a USD base and would instead surface as a spurious negative UAH base row. Agreed fix: the partner config declares the **settlement currency**; at Calculate time any base term not in that currency (in practice: overhead) converts via `fx_service.resolve()` (manual override wins, as everywhere); the settlement row stores **`fx_rate_used`** (NULL = no conversion applied); **no usable rate → loud 422**, never a silently-dropped term. The `PARTNER-SETTLEMENT-STALE` staleness recompute must use the **stored** rate, not the current one, or every NBU move falsely flags all unpaid settlements. ⑦ **Calculate-anytime cadence is a requirement** (Sergii: settling must be possible any day, even daily). Immutable snapshots + Model 2 refund dating support it, but it makes an **overlap guard** per (shop, partner) mandatory — overlapping settled periods are a double payment. Proposed: next period auto-starts at `last period_end + 1`; hard-block or loud warn on overlap (exact shape = open question for the spec). Data-readiness corollary: an Etsy month is only honestly settleable after its statement import (fees are 0 until then), and Shopify needs `SHOP-FEE-1`'s `fee_percent` actually set — both surface via the preview panel in ⑨. ⑧ **Model shape:** a **global `partners` entity** (identity) + per-shop **`shop_partner_config`** (percent, basis, settlement currency) rather than a shop-scoped partner — the same person on two shops stays one identity with one aggregate balance; the UI still lives in shop settings. ⑨ **Settlement preview ships a "base quality" panel:** orders-missing-cost count for the period (already computed as `diagnostic.orders_missing_cost` in the finance response), closed Etsy months without an imported statement, and refund-booking status per platform. This subsumes `PARTNER-SETTLEMENT-STALE` fix (a) and turns the near-zero COGS coverage (confirmed by Sergii 2026-08-06 — costs are not yet entered into the DB, so no SQL measurement was needed) into a visible number instead of a silent overpay. Consequence for sequencing: the **turnover basis is trustworthy immediately**; the **profit basis becomes trustworthy only after COGS backfill + statement imports**, and the panel is what makes shipping the config now safe anyway. ⑩ **Accepted trade-offs to state in the spec:** `OrderRefund.amount` may include a refunded-shipping portion, so deducting it from a shipping-free base slightly over-deducts (in the business's favour — accepted); a refund landing after a partner's last settled period is never deducted from anything; a refund is deducted from the period it lands in, at that period's configured percent, not the percent in force at sale time. ⑪ **Prod check 2026-08-06 (read-only UI, Cowork subagent):** Lamamarka Shopify's `PLATFORM FEE (%)` (Shops → Config → Edit Store Settings → Platform API tab) is **empty** — SHOP-FEE-1 is still awaiting its rate. The field's own helper text pins the semantics: the fee is applied to each order **at import time and frozen there; changing the rate never re-prices existing orders**. Consequence: setting the rate today fixes only future imports — every already-imported Shopify order keeps `platform_fee` NULL and computes as fees = 0 in any fee-consuming partner base. Before the first Shopify partner settlement: (a) Sergii sets the rate (business number, his call); (b) **backfill decision: YES** (Sergii 2026-08-06 — the most recent month has not yet been settled with the partner, so corrected fees must flow into the first real settlement; shape: a one-off reviewed script/SQL that Sergii runs on dev then prod after setting the rate in the UI, never wired into sync); (c) the ⑨ base-quality panel must count missing-fee orders on **Shopify** too, not only unimported Etsy months. **Rate research (Cowork subagent 2026-08-06, sources incl. westernbid.com rate card + Shopify Help Center):** Shopify **Grow** ($105/mo — Sergii's "hundred-dollar plan") third-party-provider fee **1%** of order total incl. shipping + WesternBid **4%** (computed on the post-PayPal remainder; their FAQ explicitly says the PayPal fee is NOT included) + PayPal US-tier **1.9% + $0.30** → effective ~7.18% on an $80 order … ~7.54% on $40 → **recommended flat 7.5%**; use 8.0–8.5% if Stripe or the non-US PayPal tier (2.9% + $0.30) turns out to apply. Verify before locking in: one real WB settlement statement against a known order, and Shopify admin → Billing for the 1%. ⑫ **`PARTNER-SETTLEMENT-STALE` fix (b) is PROMOTED INTO this sprint's scope.** With Calculate-anytime cadence, refund/fee data that arrives after a window was settled (Etsy statements land monthly; Shopify refund sync is daily) silently changes an already-settled base — amendment ④ deducts a refund in its dated period, but a refund dated inside an ALREADY-settled window would otherwise escape deduction entirely. The guard: a read-only recompute-and-flag on **UNPAID** settlements (same formula, **stored** `fx_rate_used`), surfaced as a staleness badge; delete-and-recalculate remains the remedy. Paid settlements are never recomputed. This closes both halves of `PARTNER-SETTLEMENT-STALE` (a → the ⑨ panel, b → this). |
| PARTNER-CONFIG-1-superseded | ~~Original scoping note~~ | **Raised by Sergii 2026-08-06 as the next substantial piece of work, deliberately parked to finish the tracking line first.** Requirement as stated: there are several partners and the number will grow; **each carries his own percentage**; and crucially **the basis differs per partner** — with some he settles on a share of net revenue, with others on turnover. Today PART-1 supports none of that structurally. `partner_name` is a free `String(200)` typed in at settlement time on both `PartnerSettlement` and `PartnerPayment` (no partner entity, no account, no list, so a typo silently creates a second partner and splits their balance). The percentage is not configuration either — it is entered per settlement and frozen into that row. And only **two** bases exist (`PartnerSettlementFormula`): `REVENUE_ITEMS_MINUS_FEES` and `NET_PROFIT_PRODUCT_ONLY` — **neither is plain turnover**, so the "по обороту" partner cannot be modelled at all right now. Design work needed before any code: a partner entity with default percent + default basis, whether a settlement may override the defaults (probably yes — it already snapshots both, which is the right instinct), what "turnover" means precisely (`total_price` incl. shipping, or items revenue gross of fees), whether partners are shop-scoped or global, and what happens to the existing free-text settlements on migration. Keep the immutability invariant — settlements are historical facts. Related: `PARTNER-SETTLEMENT-STALE`, `STATEMENT-MONTHLY-GUARD`. |
| STATEMENT-MONTHLY-GUARD | Nothing warns when a closed month has no imported Etsy statement | Raised by Sergii 2026-08-06, right after the first production import, and **explicitly preferred over a calendar reminder** — a check inside OrderHub cannot be ignored, snoozed or forgotten on holiday. The gap: statements are monthly and arrive after the month closes, so orders carry `platform_fee IS NULL` until their period is imported. Nothing surfaces that. A skipped month silently returns the system to the state this import just fixed — profit overstated on every order of that month — and it fails **silently**, which is the worst property a money gap can have. **The argument that makes it more than hygiene:** partner settlements use `items_revenue − non_shipping_fees` and are **immutable snapshots** (`partner_payout_service.py:9`). Calculating a settlement for a period whose statement is not yet imported computes the partner's share against **zero fees** and therefore **overpays** — real money leaving the business, not merely a misreported number, and unwindable only by deleting the settlement and redoing it. Shape: a dashboard warning (mirrors the existing low-stock packaging card) listing each closed calendar month with Etsy orders but no `etsy_statement_line` rows for that `period_month`, per Etsy shop. Cheap — one query over `orders` vs `etsy_statement_line`. Should also gate or warn at settlement time. Related: `ETSY-REFUNDS`, `PARTNER-SETTLEMENT-STALE`. **Update 2026-08-07: the settlement-time half shipped in PARTNER-CONFIG-1** — the preview base-quality panel lists closed Etsy months without statement rows and Shopify NULL-fee orders. What remains open, and is now this row's whole scope, is the **dashboard half**: surfacing a missing month without anyone opening the settlement page (the low-stock-card mirror). |
| PARTNER-SETTLEMENT-STALE | **CLOSED 2026-08-07 by PARTNER-CONFIG-1** — (a) the preview base-quality panel warns on unimported months/fees, (b) the staleness recompute flags unpaid settlements whose base diverges from the snapshot (stored `fx_rate_used`; paid rows never recomputed). ~~A settlement computed before its period's fees are booked silently overpays, and nothing detects it~~ | Surfaced 2026-08-06 while explaining PART-1 after the first statement import. Settlements are **immutable by design** (`partner_payout_service.py:9` — "Settlements and payments are IMMUTABLE post-create. No update path.") and store `computed_amount` as a snapshot of `base × percent / 100` at the moment Calculate ran. That is the right design — a settlement is a historical fact, not a live view. But it means the ordering **statement import → then settle** is load-bearing and enforced by nothing: settle first and the partner's share is computed on `fees = 0`, i.e. too high, and the only remedy is delete-and-recalculate (`delete_settlement` exists, and the row carries a delete action). No warning exists at calculation time, and no check flags an existing settlement whose period's fees changed afterwards. Two possible fixes, not exclusive: (a) at preview time, warn when any month in the requested period has no imported statement — folds naturally into `STATEMENT-MONTHLY-GUARD`; (b) a staleness check that recomputes each unpaid settlement's base and flags any that no longer matches its snapshot, which is a read-only comparison and would also catch a period re-import. Note (b) must apply to **unpaid** settlements only — a paid one is settled history and must not be second-guessed by a recompute. |
...
| EXPENSE-ANALYTICS-1 | Non-material shop expenses: no per-type statistics, and truly shared workshop costs have no split-across-shops story | Raised by Sergii 2026-08-06 during the PARTNER-CONFIG-1 design review; parked deliberately to keep focus on partner config. **What exists:** `OverheadMaterial` (name/unit only — **no category field**) + `OverheadMaterialReceipt` with optional `shop_id` (NULL = global/unallocated), surfaced only as the single allocated-overhead card in Finance per shop/period, the unallocated card on the dashboard, and a per-item receipts list. Ads and Shopify subscriptions **are recordable today** as overhead items per shop — the gap is analytics (no breakdown by expense type, no time-series, no cross-shop comparison), not capture. **Second half — central purchase, per-project use:** BOM materials (leather, plywood) already allocate correctly — the receipt is central, consumption at SHIPPED books cost onto each shop's orders, so no work needed there beyond BOM coverage. Packaging boxes do **not** (no cost model — that is `MAT-7`). Truly shared overhead has **no allocation rule at all**: NULL-shop receipts stay out of every shop's P&L and out of partner bases (PARTNER-CONFIG-1 amendment: only shop-allocated overhead enters a partner base — same as Finance today). Any allocation-key design (e.g. revenue-proportional split of shared costs) should come **after** PARTNER-CONFIG-1 ships, since it would change partner bases retroactively-looking-forward. Likely shape when picked up: a `category` field on `OverheadMaterial` + an expenses view (by type × period × shop) before any allocation engine. Related: `MAT-7`, `NETPROFIT-RECONCILE`. |
| PARTNER-CONFIG-1-followup-1 | Saved settlements never surface `fx_rate_used` | Found in the FX retest smoke (2026-08-07). The frozen rate is visible only in the live preview ("Converted at 44.7626 UAH/USD — this rate is frozen onto the settlement"); the saved row has no rate column, no tooltip, and no detail view — the one number that explains a UAH-overhead settlement's arithmetic disappears after Save. Same surface nit: the breakdown label reads `allocated overhead (UAH)` while the value shown is USD — source-currency label on a converted number. Small UI additions; bundle with the next partner-surface sprint. |
| PARTNER-CONFIG-1-followup-2 | TURNOVER preview reuses the PROFIT missing-cost warning copy | Smoke 2026-08-07: a turnover base ignores COGS by definition, yet its preview panel says "A profit base treats their COGS as zero, which overstates it". Misleading noise on the one basis it cannot affect. Fix: per-basis warning copy (suppress or reword the missing-cost item for TURNOVER). Tiny. |
| PARTNER-CONFIG-1-followup-3 | "Check for changes" staleness button is a silent no-op when nothing is stale | Smoke 2026-08-07: clicking produced no toast, no state change — correct result (nothing stale), indistinguishable from a dead button. Add an "everything matches" confirmation toast. Related note kept here rather than its own row: partner **entities** live on after their last shop config is removed and there is no global partners admin screen — accepted for now (harmless rows, balances still aggregate), revisit if entity clutter becomes real. |
| OVERHEAD-RECEIPT-DELETE | Overhead receipts have no delete/edit in the UI | Pre-existing gap surfaced by the FX retest (2026-08-07): the receipts table has no actions column, and **archiving the parent item does not detach its receipts** from shop finance — dev now carries a residual 1 000-UAH smoke receipt on Lamamarka Shopify (June 2026) until removed by SQL (see the PARTNER-CONFIG-1 runbook ⑥). Consequence: a wrongly-entered expense is permanent without DB access, and it flows into allocated overhead → Finance net profit → partner PROFIT bases. Fix: backend delete (probably soft/audited, given the money surfaces it feeds) + confirm UI. Decide the audit story before building — deleting booked overhead changes historical P&L. |
...
| FX-AUDIT-SEMANTICS | `fx_rate_audit` write condition is **unknown** — two deploys behaved differently | **Rewritten 2026-08-04 (second deploy) — the original claim here was wrong and is retracted.** As first filed, this row asserted the FX refresh writes three rows on every backend start. The STATEMENT-IMPORT deploy falsified that: the scheduled job ran and succeeded at 15:23:37, and wrote **zero** rows — the table stayed at 9 and `fx_fetched_at` stayed at 12:04:04. Yet the earlier deploy the same day took it 6 → 9 **with the rate unchanged** (44.7876 → 44.7876). Both observations are first-hand; they disagree. So the honest state is: **the write is guarded by some condition, and nobody has established what it is** — plausibly a date rollover, a first-fetch-of-day path, or a difference between the boot path and the scheduled path. **Whoever picks this up must determine the actual condition from the code before proposing a fix**, and must not trust either observation as the general rule. The original concern (a table named "audit" that records fetches rather than changes makes "when did the rate actually change?" a diff rather than a read) may still hold or may be a non-issue — that depends on the answer. Low-priority; bundle with any FX-touching work. `historical-COGS-recompute` does **not** depend on this — NBU's `date=` parameter supplies per-date rates directly. |
...
| WB-TRACK-1-followup-1 | A retry window before code 111 counts as a `problem` | From `WB-TRACK-1` (2026-08-05). Code 111 ("failed delivery, recipient absent") is classified as `problem` **immediately**, deliberately: a false alarm costs one glance, a missed failed delivery costs the parcel when it returns to sender. But couriers commonly retry the next day, and NP Global hands the last mile to a US carrier whose retry behaviour we have **never observed** — so a delay threshold today would encode a guess, which is the failure mode this project has paid for twice. The transition log (`wb_tracking_event`, one row per status change) is now collecting exactly the evidence needed: how often 111 resolves within 24h without intervention. Revisit once there are enough 111s to see a pattern; one parcel (Excelsior, since 05-08 02:10) is not a distribution. Refining this is **enabled**, not pending. **BLOCKED 2026-08-25, and not on data volume — on cadence.** `WB-TRACK-EVENT-COMPLETENESS` found the poll runs **once a day** and the NP API returns no history, so "how often does 111 resolve within 24h without intervention" is a question the current sampling rate **cannot answer in principle**: a 111 that clears inside a poll gap is never recorded, and the ones we do see are biased toward the slow-resolving tail. Waiting for more 111s will not help. Depends on `WB-POLL-CADENCE`; revisit only after ~2 weeks of data at the higher rate. |
| WB-2 | Order ↔ parcel matching — and it now blocks a workflow, not just convenience | Designed in `WB-1` (2026-07-21), deferred as "blocked on real parcel data". **Re-weighted 2026-08-06 after `WB-TRACK-2` went live: every single row on the monitoring page renders `Not linked`.** `wb_parcel.order_id` is populated only when a manager fetches a WB label through OrderHub (`shipping.py:609-615`), so in practice it is empty. The consequence is concrete: the page's most actionable finding is "parcel arrived, recipient has not collected it — write to the customer", and the page gives a recipient name and a city but **no order and no email**. The tool shows the problem and withholds the means to act on it. `parcels/sent` returns no street, city, email or order reference, so matching must be built on `RecipientName` + `RecipientPostalCode` + `RecipientCountryCode` with a manual-review queue; `RecipientPhone` is accepted as a filter though never returned, making it the tie-breaker. Sequence after the `no_data` question settles, since that may change what the page needs. **UNBLOCKED 2026-08-20** — the `no_data` question is settled (transient, see `WB-TRACK-NO-DATA-TERMINAL`); with 17/19 attention rows now overdue-moving and many known to be "arrived, awaiting collection", the missing order/email link is the main obstacle to acting on the page's findings. Next candidate together with `WB-TRACK-ARRIVED-STATE`, after `WB-ALERTS-1`. **Confirmed 2026-08-24 weekly:** overdue-moving held at 17/24 = **71% of in-flight** (identical ratio to the 08-20 baseline), but the tail aged — parcels >7d overdue went **8 → 13 of 17**, i.e. the same parcels sitting still rather than churning. Priority unchanged, case mildly strengthened. Real-world confirmation of what the missing link costs: acting on the two 08-25 parcel investigations required the WB cabinet, an email thread and the manager, because the page gives no order and no email. |
| WB-TRACK-NO-DATA-TERMINAL | Is `no_data` an anomaly or the normal end of Nova Poshta's visibility? | Surfaced by the first scheduled poll (2026-08-06), which took `no_data` from 1 parcel to 7 in one run. **The six new ones went dark simultaneously**, all last seen ~1.7d earlier, all with a last status of "Відправлення прямує до <city>" — a pattern that looks far more like a handoff boundary than six independent failures. Hypothesis to test: NP stops resolving an export once the US carrier takes over. **If true, two things follow and both are bad.** `no_data` accumulates every week, so the attention group bloats and the 19%-flagged selectivity that justified the exception-first design erodes. And a parcel that **actually arrived** sits in "needs attention" permanently, because nothing will ever move it to delivered — a false alarm that never clears, which is worse than no signal at all. **One design branch is now closed by evidence (2026-08-06).** The planning agent proposed that WesternBid might hold the onward US carrier number, and that our 3-day mirror window (`WB_POLL_WINDOW_DAYS`) was hiding it — i.e. that we were blind by our own design. **That was wrong, and was disproven read-only before any work started.** Across 109 parcels spanning 45 days, **not one carries more than two tracking elements**: each shipping type is issued its carrier number at creation and never gains another. The USPS `9261…` on the ConsolidationOptimum parcel is that type's own number from creation (filed under `Identifier: UPS` — the field is a label, not a carrier, per `models/wb_tracking.py:54-57`), not something added at handoff. WB's status vocabulary is creation-time only — `Parcel created` ×101, `Sent from Ukraine warehouse` ×6, `Parcel canceled` ×2 — and **zero** parcels have a live status differing from our mirror. The delivered control still reads `Parcel created` live, four days after Nova Poshta recorded the handover. So WB never learns the outcome, widening the poll window would gain nothing, and **there is no onward tracking number available from any source we already have**. What remains: age-out, a manual "resolved" action, or a third-party aggregator (17track / AfterShip), which is now the ONLY data-bearing option and would also close the 7 permanently-untracked UPS/USPS parcels in the same move — a stronger case for it than when it was first weighed and deprioritised. Cheapest remaining test of the hypothesis itself: whether any dark parcel ever returns to a live status — the transition log answers this by itself, and a review is scheduled for 2026-08-20. **CLOSED 2026-08-20 — the scheduled review ran and FALSIFIED the hypothesis.** Prod queries (CC, read-only): 45 parcels ever went dark, **43 recovered** (30 since delivered), dark spells last 1–5 days (avg 2.15); the 2026-08-06 six all recovered, five delivered; the code-80 parcel `59500007112662` resumed 08-07 and delivered 08-09. `no_data` is **episodic feed blindness** (stub codes 80/**81 (new)**/122), self-clearing: 7 → 2 with zero aged_out and zero manual action. **Split and manual-resolve both rejected.** Kept: an age threshold at >6 days dark (recovery ceiling 5.0d), folded into `WB-ALERTS-1` as the `no_data_stuck` alert; on close date it selects exactly one parcel, `59500007135457` (dark 9.7d, its second episode — manager asked to check it in the WB cabinet once). Composition note: needs-attention is now 17/19 overdue-moving (71% of in-flight vs 19% at baseline) — `WB-TRACK-ARRIVED-STATE` + `WB-2` are the next levers. Full record: `docs/reviews/2026-08-20-wb-track-no-data-checkpoint.md`. **Footnote 2026-08-25 — the closure stands, with one documented exception.** `59500007135457` (the parcel this closure flagged at 9.7d) reached **14.7 days dark**, ~3× the 5.0d recovery ceiling the self-clearing model was built on. Investigated by hand in the WB cabinet: it is **not** a long feed outage — the parcel genuinely has not moved since "The plane arrived at New York" on 09.08 22:42, 16 days, against ~2 days NY→US-carrier on a sibling parcel. So the exception does not weaken the model; it means a long-dark parcel can also be a *stuck* parcel, and >6d dark is the right place to look rather than something to explain away. Beware the WB cabinet's two timestamps: «Дані статусу» is the newest event, «Останнє оновлення статусу» is only the poll time — reading the latter as movement is the natural mistake. Details: `docs/reviews/weekly-parcel-health-2026-08-24.md` § Addendum. **Second footnote, same day — the 5.0d recovery ceiling is a FLOOR, not a measurement.** `WB-TRACK-EVENT-COMPLETENESS` established that the poll runs once a day and the NP API carries no history, so a parcel that went dark and recovered inside one poll gap **never entered the sample**. The closure's verdict is unaffected — 43 recoveries out of 45 is not a marginal call — but quote "1–5 days, avg 2.15" as a lower bound on darkness duration and an undercount of episodes. The `no_data_stuck` threshold at >6d survives this: a floor on observed spells argues for the threshold being conservative, not wrong. |
| WB-ALERTS-1 | Parcel alerts on the dashboard + usage note on `/westernbid` — push instead of pull | **DONE 2026-08-20** — merged `ad2f441`, deployed, smoked on dev + prod; see the closure entry above. Row kept so the table stays exhaustive. Original scoping note follows. Filed 2026-08-20 from the review's second finding: **in two weeks nobody — manager or owner — opened `/westernbid` even once.** A pull surface does not get pulled; the page's classification is trustworthy (see `WB-TRACK-NO-DATA-TERMINAL`) but its findings reach no one. Decision (Sergii, 2026-08-20; Telegram considered and rejected — messages drown among others): high-signal alerts surface **on the dashboard (main page)**, prominent but collapsible/compact, and each is cleared either **automatically when the condition disappears** (load-bearing — without auto-resolve the dashboard becomes the next ignored surface) or **manually by OWNER/MANAGER with who+when recorded**. Four alert kinds, each mapping to one action: `failed_delivery` (code 111, immediate), `no_data_stuck` (dark >6d — recovery ceiling is 5.0d), `overdue_long` (>7d past NP's promise; deliberately not every overdue), `untracked_aging` (no NP number, >14d old; ~6 launch alerts from the existing backlog, dismissed once). Thresholds app_settings-overridable (`DEFAULT_STALLED_DAYS` pattern). Dedupe: one open alert per (parcel, kind); dismissals don't resurrect, but a cleared-then-recurring condition is a new episode and re-raises (`59500007135457` did exactly this). Generation runs **inside the `run_poll` pass** (one definition, two callers — scheduler + manual refresh), reusing `classify_parcels` logic, never duplicating it. Second deliverable: extended collapsible Ukrainian "Як користуватись" note on `/westernbid` (Sergii: a human still needs to be able to open the page cold and understand it). `task.md` written 2026-08-20 with 5 open questions for the planner (hook point + transaction boundaries, table shape, reuse check, dashboard conventions, the honest bound on `untracked_aging`). Companion process change: a weekly Cowork parcel-health review (Mon 10:00) replaces the one-off 2026-08-20 checkpoint. |
| WB-TRACK-ARRIVED-STATE | "Overdue" conflates *stuck in transit* with *arrived, awaiting collection* | Observed on the live page 2026-08-06. Many overdue parcels read "Відправлення у Pensacola. Очікуйте повідомлення про прибуття" (also Chandler, Tallahassee, Palm Springs) — the parcel is **at destination** and waiting for the recipient. That is still actionable, but the action is "nudge the customer", not "find the parcel", and mixing the two makes the attention list harder to work. Possible refinement: a distinct `arrived` state keyed off the relevant `StatusCode`s, shown separately from overdue-in-transit. Needs the status-code semantics settled first (`WB-TRACK-1` OQ2 covered the classification, not this distinction), and needs enough observations to confirm which codes actually mean "at destination". Low priority until the attention list is genuinely hard to work. **2026-08-24 weekly:** still 71% of in-flight is overdue-moving and the >7d tail deepened 8 → 13, so the conflation persists; priority unchanged, sequenced with `WB-2`. |
| WB-TRACK-POLL-FLOOR | No minimum interval between Nova Poshta polls on backend restart | Raised by CC at the `WB-TRACK-2` deploy (2026-08-06) after the `next_run_time` fix made the poll fire on every startup. Unlike the FX refresh, which has an `FX_MIN_REFETCH_HOURS` floor, the tracking poll has none — so a crash-looping backend would hit Nova Poshta once per restart. Judged acceptable at the time and the reasoning was documented in the code: one batched keyless request, no per-shop credential or quota, and events are written only on a genuine status change, so a repeated poll is close to free. Filed because "acceptable given current assumptions" is worth writing down rather than rediscovering, and because a hard floor is a small change if the assumption ever stops holding. |
| WB-TRACK-CODE-80 | Nova Poshta stub codes — a tracking number that stops resolving mid-journey | From `WB-TRACK-1` (2026-08-05). `59500007112662` went from code 5 ("прямує до Garnet Valley") to a 9-field stub — empty status, no dates — **during sprint planning**, and has not recovered on either dev or prod since. WB's own mirror still shows it as `Parcel created / Paid`. Handled safely (last known status kept, flagged, polling continues, transition logged) but **not understood**: NP documents nothing for 80, so whether it means number-reassigned, waybill-deleted-by-sender or data-purged-on-a-schedule is unknown, and the UI label is deliberately neutral rather than overclaiming. Cheapest path to an answer is looking up this one parcel in the WesternBid cabinet. Until then, watch whether a second parcel ever does this — a one-off and a recurring class need different responses. **Downgraded 2026-08-20:** it is a recurring class and it self-heals — the 2026-08-20 review found stub episodes across 45 parcels (codes 80 ×33, 122 ×31, **81 ×11** — 81 newly observed), 43 recovered within 1–5 days, and `59500007112662` itself resumed 08-07 and **delivered 08-09**. Stubs are episodic NP feed blindness, not number-reassignment. No action; the `no_data_stuck` alert in `WB-ALERTS-1` covers the residue. Row kept for the code inventory. **Inventory update 2026-08-24 weekly:** stub events since 08-20 were **122 ×19 and 81 ×7** — **no new codes**, and code 80 did not appear at all. Vocabulary remains 80 / 81 / 122. **Caveat added 2026-08-25 (`WB-TRACK-EVENT-COMPLETENESS`): this inventory is a FLOOR on both counts.** The poll runs daily and the API carries no history, so a stub episode that opened and closed between two polls is invisible — both the code vocabulary and each code's frequency are lower bounds. A fourth stub code may well exist unobserved. Re-measure after `WB-POLL-CADENCE`. |
| WB-ALERTS-2 | `untracked_aging` alerts have no self-clearing path and will accumulate as permanent noise | Filed 2026-08-24 from the first weekly after `WB-ALERTS-1` shipped. The auto-resolve mechanism is what makes the dashboard block survivable — an alert clears when its condition disappears (proven: 1 `overdue_long` auto-cleared in week one). **`untracked_aging` structurally cannot participate:** those parcels have no NP number and never will, so tracking data never arrives, the condition never disappears, and the alert can only leave via manual dismissal or the 60-day age-out. They are **7 of 23 open alerts (30%)** today and every new UPS/USPS parcel adds one permanently for 60 days. The risk is precise and is the one this whole feature exists to avoid: a third of the block is undismissable-by-events, so the operator learns the block is stale and stops reading it — killing the 4 kinds that *do* self-clear along with it. Options (not decided): a separate collapsed group with a count rather than individual rows; suppression after one acknowledgement; or accept dismissal as the intended lifecycle for this kind and say so in the UI. **Do not build before migration** — the right answer depends on whether the operator experiences 7 rows as noise or as a useful to-do, which cannot be known while nobody uses the dashboard. Revisit in the first weekly after migration. |
| WB-TRACK-EVENT-COMPLETENESS | Our tracking mirror may hold fewer events than Nova Poshta's own site shows | Found 2026-08-25 while investigating `59500007135457` by hand. Our DB's last resolved status is code **120** "Митне оформлення завершено" (09.08 13:50); NP's public tracking page shows **two later events the same day** — "Cleared customs, in transit" (19:02) and "The plane arrived at New York" (22:42) — and `no_data_since` starts right after 13:50. Two candidate explanations, and they have different consequences: either **(a)** `getStatusDocuments` returns a thinner event set than the web UI, in which case every parcel's history is systematically truncated and the poll cannot see what the cabinet sees; or **(b)** the API simply went to stubs between 13:50 and 19:02 and we lost only that parcel's tail, which is benign. **Why it matters beyond one parcel:** every weekly health report, the `no_data` recovery-ceiling statistic (5.0d) and the `WB-TRACK-CODE-80` stub inventory are all computed from `wb_tracking_event`. If the mirror is lossy, those numbers are floors, not measurements. Cheap read-only check: call the API for 2–3 parcels whose cabinet history is known (start with `59500007135457` and `59500007140095`) and diff event-for-event against the page. No code change until the answer is known. Investigation handed to CC 2026-08-25 via `task.md`. **ANSWERED 2026-08-25 — verdict (a) SYSTEMATIC. Investigation by CC, read-only, no code written.** **The API carries no history at all.** `getStatusDocuments` returns **one flat current-status record** per number — the live response for `59500007135457` is 9 keys (`StatusCode: "81"`, empty `Status`), while NP's public page shows all 11 events for the same parcel at the same moment. Across all 36 in-flight parcels in one batch: **not one non-empty list/dict field in any record**; the only array-typed keys (`Packaging`, `PartialReturnGoods`, `UndeliveryReasons`) are empty and none is a route list; the only history-shaped key is `DateScan`, a scalar. Five plausible history methods (`getStatusDocumentsHistory`, `getDocumentsEWMovement`, `getStatusDocumentsMovement`, `getDocumentTracking`, `getTrackingHistory`) all return `success: false, "User is undefined"` — **the keyless surface exposes exactly one method and it is current-status only.** So `wb_tracking_event` is **by construction a poll-sampled approximation** of NP's history, not a mirror of it. **Three independent mechanisms thin it, and they must be kept apart:** (1) **no history in the API** — structural, NP's; (2) **poll cadence** — the job is `interval, days=1` (`backend/scheduler.py:463-470`), so any two status changes inside 24h collapse into one (observed: GoFo handover 12.08 17:37 → depot 17:47, ten minutes apart, one survived; the paired `Return` events likewise) — **this one is ours, not NP's**; (3) **API lateness and omission** — 41 min after the 09.08 19:02 event the API still returned 13:50, and 48 min after the 11.08 18:55 event it still returned 10.08 19:31 (both could be propagation delay), but **the 14.08 Carteret NJ depot arrival never appeared across nine consecutive daily polls** while the cabinet had it — that single line is what rules out explanation (b), since it is neither a stub window nor a poll gap. **Measured loss:** `59500007135457` cabinet 11 events / mirror **1**; `59500007140095` cabinet 9 / mirror **3**; delivered control `59500007140177` cabinet 10 over 12 days = **0.83 events/day**, the ceiling case. Fleet-wide: 114 parcels / 519 events, mean **4.55/parcel** (3.67 excluding stubs), median 4, and **0.64–0.97 events/day — at or below the 1/day cadence ceiling**. **Do not quote "⅓ of the cabinet's density" as a measurement** (Cowork's paraphrase, corrected 2026-08-25): it compares the fleet rate against **one parcel's busiest leg** (11 cabinet events in ~4 days ≈ 2.75/day, a customs-and-transfer stretch), and events arrive in bursts, so a lifecycle average is far below a burst rate. The delivered control's 0.83 events/day is our **stored** count and was **never diffed against its own cabinet history** — it measures our ceiling, not the loss. The fleet-wide ratio is **unmeasured**; firming it up means diffing two or three full lifecycles against the cabinet by hand, not more aggregate arithmetic. Staleness when first recorded: 40.9% <6h, 41.6% 6–24h (one poll cycle), 12.2% 24–48h, 5.3% >2d; of the 87 events already >24h stale, 42 are a parcel's pre-existing first event, 17 emerged from a stub window, and **14 are plain API lateness with the poller running normally**. Where we *did* capture an event, `np_tracking_update_date` matches the cabinet **to the minute** — the mirror is accurate about what it holds; it just holds less. **Consequence, and it differs per statistic — this is the part to carry forward:** **state counts are unaffected** (`wb_parcel_tracking` holds the current row, and a parcel's present status is as true as the feed itself), but **everything derived from `wb_tracking_event` must be re-characterised as a FLOOR, not a measurement** — the 5.0d `no_data` recovery ceiling is the longest dark spell observable at daily resolution (a dark-and-recovered spell inside one poll gap never entered the sample), the 80/81/122 stub inventory is a lower bound on both the vocabulary and each code's frequency, and per-parcel event counts run ~⅓ of ground truth. None of these numbers is wrong; each is a floor being quoted as a measurement. **Cowork's own error, corrected:** this row and the `task.md` that drove the investigation both said "the 4h poll" — there is no 4h poll; that figure was conflated with `NP-FIX-5`'s *designed* 4h domestic batch, which was never built. The cadence mechanism is invisible while that premise stands, which is exactly why CC flagged it. Follow-up filed: `WB-POLL-CADENCE`. Full evidence: `docs/reviews/weekly-parcel-health-2026-08-24.md` § "CC investigation". |
| WB-POLL-CADENCE | The WB tracking poll runs **once a day**: current status is routinely a full cycle stale, and intra-day changes are lost permanently | Filed 2026-08-25 from `WB-TRACK-EVENT-COMPLETENESS`. **Attribution first, because the obvious framing is wrong.** Cadence is **not** the dominant loss mechanism. Attributing the 16 misses in CC's two hand-diffed parcels: **stub windows 9 (56%)**, API lateness 3, API omission 1, **cadence 3 (19%)**. Fixing the cadence would not have recovered the other 13 — the API was returning stubs or had not published the event at all. (Cadence is a *sufficient* cause more often than a *binding* one: the 08-07 cluster had four changes inside one poll gap, so cadence alone would have cost three of them even without the stub — but the stub is what actually bound. Do not claim the fix recovers those.) `backend/scheduler.py:463-470` registers `run_wb_tracking_poll` as `interval, days=1`. Nova Poshta publishes roughly **2.75 status changes per day** on a moving parcel (measured on `59500007135457`: 11 events in 4 days); we observe **0.64–0.97/day**. Since the API returns only a current status, every change that happens between two polls is **lost permanently** — there is no backfill, ever. **Why this is worth more than tidier statistics.** (1) `WB-TRACK-1`'s founding asymmetry — "a missed failed delivery costs the parcel" — is what justified raising code `111` to `problem` immediately with no retry window; at daily cadence we can be **up to 24h late even noticing 111**, which quietly spends most of the margin that argument was defending. (2) `WB-TRACK-1-followup-1` is waiting on evidence of "how often does 111 resolve within 24h without intervention" — **at 1 observation/day that evidence cannot be collected in principle**, so that row is blocked on a measurement the current cadence makes impossible. (3) The `no_data` recovery ceiling (5.0d) and the stub-code inventory are floors for the same reason. **The cost of fixing it is near zero and this is already documented in the code:** the poll is one batched keyless request with no per-shop credential or quota, and `record_poll` writes an event **only on an observed change**, so re-polling an unchanged parcel writes nothing (`scheduler.py:459-462`). Six polls a day is six requests a day. **Proposed:** change `days=1` to `hours=4` (matching what the 2026-05 NP audit chose for the domestic poll, `docs/integrations/nova-poshta-audit-2026-05.md:322`), ideally behind a setting rather than a literal. **Expected effect, stated honestly.** The load-bearing gain is **freshness, not completeness**: 41.6% of all 418 recorded events were **6–24h stale when first seen — exactly one poll cycle**, the one term cadence controls, and that is a fleet-wide measurement rather than a two-parcel anecdote. At 4h most of that bucket moves under 6h, and 111 detection latency drops from ≤24h to ≤4h. Event density improves too, but **modestly** — cadence bound only ~19% of observed misses — so do **not** promise ~3×. A fourth benefit is the cheapest way to close the biggest remaining unknown about the feed: **whether stub windows are continuous outages or per-request flicker.** At 1 sample/day that is unanswerable; at 6/day, a window that yields a resolved status on some polls and stubs on others proves flicker, and flicker would mean much of the 56% stub-window loss is recoverable by sampling alone. Derived statistics stop being floors only for data collected **after** the change (~2 weeks to be usable), and the historical numbers never retro-fix. **Open question before building:** whether to also make the interval app_settings-overridable (the `DEFAULT_STALLED_DAYS` pattern) or keep it a constant; and confirm `WB-TRACK-POLL-FLOOR`'s reasoning still holds at 6×/day (it should — the restart-storm cost is unchanged and still free). Small: one scheduler line plus a test. **Note the old numbers do not retro-fix** — density improves only for parcels polled after the change. **DONE 2026-08-25 — merged `42002cb` (docs `b23beb1`), NOT YET DEPLOYED** (`scheduler.py` is baked into the image, so prod needs `git pull` + `up -d --build backend`). `days=1` → `hours=4` at `scheduler.py:481`; `test_scheduler.py:172` verdict corrected to "startup: 4-hourly"; three stale "daily" prose refs fixed in `routers/westernbid.py` (:145, :185, :194); full suite **1076 passed, exactly the baseline**. **OQ2 answered:** the whole in-flight set is still **one** batched request — `NP_TRACKING_BATCH_SIZE = 100` against 34 parcels in flight — so this is 6 keyless requests/day, not 6× anything; `WB-TRACK-POLL-FLOOR`'s reasoning is unchanged. 4h also sits inside NP's own recommended ≥4–6h per shipment. **The lesson worth keeping, and it is a familiar one:** the daily interval was not an oversight — it was justified in the code by "p90 of observed scan gaps is 1.17 days, so daily loses nothing". That statistic was computed from **our own daily-sampled mirror**, which cannot produce an answer below ~1 day no matter what the feed does. **A self-confirming measurement**, same family as `npx tsc --noEmit` checking zero files and the scheduler test that asserted only `commit` — an instrument that reports the state of its own blindness and is believed. The comment block now records it as disproven rather than deleting it. **DEPLOYED 2026-08-25** — `git pull --ff-only` (clean fast-forward `ad2f441` → `b23beb1`), `up -d --build backend`; other services untouched, the three server-only overlay files left untracked as documented. APScheduler confirms the trigger directly: `run_wb_tracking_poll (trigger: interval[4:00:00])`, all other jobs still `interval[1 day]`; the boot fire ran immediately (rule 2 held) with `polled=34` — **one batched request, exactly as OQ2 predicted**. First evidence of the cadence in the data: polls at 08-24 14:51 (21 rows), 08-25 14:51 (19 rows), 08-25 19:55 boot-fire (**5 rows, 5 hours after the previous poll instead of 24**) — five status changes to code 119 that the old cadence would have held until the next day, and any of them overtaken by a later status would have been lost permanently. **Formal two-poll-gap evidence deferred** to the 20:55:37 UTC run (~4h later); registration + boot fire are what is proven at deploy time. **Early signal in the same summary:** `no_data=9`, down from 11 on 08-24 — consistent with the 2026-08-24 weekly's prediction that the nine-parcel 0.9-day dark batch would self-clear. `alerts_opened=0 alerts_resolved=0`, so the `WB-NO-DATA-CLOCK-RESET` canary (`59500007135457`) survived the first poll at the new cadence. |
| WB-NO-DATA-CLOCK-RESET | One resolved poll inside a dark spell resets the `no_data_stuck` clock to zero — and raising the cadence makes it fire more often | Filed 2026-08-25 by CC during `WB-POLL-CADENCE` (option A: reported, deliberately not fixed in that pass). `record_poll` clears `no_data_since` **unconditionally on any resolved poll** (`wb_tracking_service.py:458`) and re-stamps it to `now()` on the next stub (`:435-436`). So "days dark" is measured from the **most recent** stub onset, not from the first — and a parcel whose feed flickers never accumulates 6 days, however long it has actually been unreadable. **Prod evidence that the feed already flickers at 1 poll/day:** of 66 parcels that ever went dark, **41 had two or more separate dark episodes**, and the modal dark→resolved turnaround is **exactly 24.0h in 35 cases** — i.e. pinned to the cadence floor, which means shorter flickers exist and are simply invisible today. At 6 polls/day we will see more of them, each one resetting a clock. **The concrete risk:** `59500007135457` — stub for 15 days straight, the single parcel the `no_data_stuck` alert correctly identified as genuinely stuck — would **lose its alert to a single resolved response**, which would auto-resolve as `cleared` while nothing about the parcel changed. Treat that parcel as the canary after deploy. **Why it was right not to fix it blind:** the flicker-vs-outage question is exactly what the new cadence measures, and the fix differs by answer. Options to weigh with that data, not before: (a) clear `no_data_since` only after N consecutive resolved polls; (b) keep the first-onset timestamp and treat flicker as one continuous episode; (c) **stop keying "stuck" on data availability at all and key it on movement** — the model already carries `np_last_movement_at` (`models/wb_tracking.py:68`), so a "status has not advanced in N days" clock needs no migration and is immune to flicker by construction; verify it is populated reliably before relying on it. Option (c) also subsumes the case the current alert cannot see at all: a parcel whose feed is perfectly healthy and whose status simply never changes. Revisit after ~2 weeks at 4h. |
| WB-CODE-102-LABEL | Code 102 "Відмова від отримання" claims a cause it cannot know — the recipient may never have seen the parcel | Found 2026-08-25 on `59500007140095`. NP code 102 is surfaced as **"Відмова від отримання"** (recipient refused), which reads as a customer decision. The actual case: the parcel was shipped to a wrong city/state (`8036 Canonbury Dr, CHICAGO, IL` instead of `Nolensville, TN`), was undeliverable, and was returned — **the recipient never had the chance to refuse anything**. An operator acting on the `delivery_problem` alert would open the wrong conversation with the customer ("why did you refuse?" instead of "our address was wrong"). This is the same class of error the `no_data` UI label deliberately avoided by staying neutral (`WB-TRACK-CODE-80`). Fix shape: render 102 as an outcome rather than a cause — "Повернення / не вручено" with the raw NP text available underneath — and check the other 13 `problem` codes for the same overclaiming. Tiny, frontend + label mapping only, no migration. Bundle with the next `/westernbid` or alerts sprint. |
...
| PROD-LOGS-EPHEMERAL | ~~`/app/logs` is not a volume mount on prod~~ **DONE 2026-08-06** | Noted during the ORDER-SHIPPING-1 deploy (2026-08-05); the pre-deploy log was preserved by hand at `~/server.log.pre-20260805` only because it was asked for. Since `backend/logs/server.log` is the *only* real log (docker stdout carries a fraction — see AI_ONBOARDING §9), a rebuild erases the evidence trail for whatever went wrong before it. **Closed 2026-08-06** — bind mount of `/app/logs` to `/home/prorder/orderhub-logs`, outside the git clone, applied with `up -d backend`, no rebuild; verified by force-recreating and watching one file hold two container generations. See the closure entry above. **The residual, which no volume can fix:** `docker compose logs` is still ephemeral because container stdout begins at container start by Docker's design — `backend/logs/server.log` is the durable record, the docker log is not. Related baseline fact: the preserved log was **not** error-free — 16 ERRORs from SSL cert-verify failures across five cycles on 2026-08-04 17:08–18:08, fully recovered, cause unestablished. Do not assume a clean baseline when diffing logs. |
| DOCS-RESTRUCT-1 | Docs restructuring: CLAUDE.md → per-domain invariant docs | **CLOSED 2026-08-21** — three docs commits (`2e30b4d`, `e4eae4d`, plus an amended third: idlaser SSE-gotcha + Status-line fixes), Cowork-authored, CC-verified (word-frequency deficit 0 over the 87 removed lines; runbook md5-identical; bash block byte-identical) and CC-committed. Driven by an SDD-practices research over practitioner sources (report outside the repo: Cowork project folder, `sdd-research-2026-08-21.md`). Key evidence: instruction-file bloat causes uniform rule-dropping, not graceful degradation (Anthropic docs, Thoughtworks "Agent instruction bloat" → Caution ring 04/2026, one measured A/B); docs referenced from a brief are read >90% of the time vs <10% for orphans; "spec-first works, spec-anchored dies" — hence the new docs describe `main` only and update at closure, never in-flight. Result: CLAUDE.md 239→157 lines; USER-ACCESS-1/2 + PARTNER-CONFIG-1 + WH-1/WH-2 gotchas and the whole ID-Laser section moved verbatim to `docs/design/access-control.md`, `docs/design/partner-config-current.md`, `docs/warehouse/warehouse-invariants.md`, `docs/integrations/idlaser.md`, with 1-2-line "before touching X — read Y" stubs left behind; AI_ONBOARDING §8 template gained "Domain docs to read"; §5 table row added; §2.1 bootstrap moved into the Cowork project Instructions (canonical copy stays in §2.1). Same round also fixed: profit-definition.md supersession banner (§1 missing Refunds term; §6/Q3/Q4 superseded by SHOPIFY-REFUNDS + PARTNER-CONFIG-1), the no-op `npx tsc --noEmit` purged from AI_ONBOARDING §8/§10 (the doc half of TYPECHECK-1's lesson), and idlaser.md's stale "locked" reference to the deleted `routers/mcp.py`. Rejected from the same research: adopting Spec Kit/Kiro/OpenSpec (all public long-term reports negative or reverted), a single CANON.md (spec-anchored death mode), a separate CHANGELOG.md (this file is it). |
| DOCS-TRACE-1 | Requirement→test traceability in critical sprints | From the 2026-08 SDD research (see DOCS-RESTRUCT-1). In sprints touching money/statuses/access: number task.md's Behaviour rules and name their pytest tests after the rule ID (`test_br4_...`) for direct requirement→test tracing. Adopt the traceability ONLY — not the EARS WHEN/SHALL syntax: zero evidence found for the syntax itself (the one production practitioner using it credits structure + property-based-test gating, not phrasing) and its ceremony cost is measured (a one-line bugfix ballooning to 16 acceptance criteria). A task.md-writing habit for Cowork; no code change; start with the next critical sprint and judge after 2-3 uses. |
| DOCS-DECISIONS-1 | Thin `docs/DECISIONS.md` index: "why X?" → where the reasoning lives | From the 2026-08 SDD research. The reasoning already exists (closure entries in this file, design docs) but is keyed by sprint ID, not by decision — answering "why capabilities and not roles?" requires already knowing it was USER-ACCESS-2. Fix: a one-line-per-decision index mapping each question to its closure entry / design doc. Deliberately NOT separate ADR files — duplication invites drift, and practitioner reports gate ADR value on on-demand retrieval (always-loaded decision docs pollute context). Fill lazily: add a line whenever a "why" question actually gets asked and answered. |
| DOCS-PLAN-ARCHIVE-1 | Archive this file's legacy sections when its size starts to hurt | From the 2026-08 SDD research. This file is ~6.3k lines and append-only; legacy sections A–D and long-closed epic blocks are candidates for a separate archive file, keeping the live sections (Active Roadmap, this table, Open Architectural Questions) lean. Evidence that append-only accumulation harms agent consumption is inferential only (no published comparison exists — the research explicitly flagged this gap), so: no urgency, act on real pain only, and never silently delete history — move it whole, leave a pointer. |
| OSH-TIMESTAMP-1 | `order_status_history.changed_at` default is frozen at DDL time — the audit trail has no real timestamps | Found by CC during CASE-1 (2026-08-26), verified on the live DB. `models/order.py:373` (and the initial migration) declare `server_default="now()"` as a plain Python string; Postgres coerced the *string literal* `'now()'` to a timestamptz **once, at table creation**, so every default-stamped row since April carries the identical fake time (`2026-04-25 17:11:54.840662+00`). The lost times are unrecoverable. Same family, same sprint-fix: `routers/orders.py:190` — `update_order` adds the history row by bare FK, so `OrderResponse.status_history` is served **stale on PATCH** (the idiom CASE-1's router copied and then fixed on its own side via relationship-based writes; see the CASE-1 closure). Fix: migration `ALTER COLUMN changed_at SET DEFAULT now()` (real `func.now()`), plus the relationship-write fix in `update_order`, plus an audit of other `server_default="now()"` string literals in the models. Small, but it is the **order audit trail** — do it soon, every day adds more rows with the fake timestamp. |
| ORDER-SEARCH-1 | Orders search cannot find an order by its own number | By-catch of the CASE-1 dev smoke (2026-08-26), pre-existing and unrelated to cases. Searching `91890_1899` returns "NO ORDERS FOUND" though the order exists in the unfiltered list; searching `1899` returns only `91890_1801`, whose number does not contain "1899" — so the search neither matches full order numbers nor substrings correctly, and what it *does* match is unclear. Finding an order by number is a basic manager operation (and the natural entry point to the new cases UI). Investigate what the search endpoint actually queries before proposing a fix. |
| CASE-1-followup-1 | Cases UX bundle: create form below the fold, optimistic status pill, empty-state collapse | Three small frontend items from the two CASE-1 smokes (2026-08-26), none blocking. (1) The "+ Питання" trigger sits at the top of the section but the form renders at the *bottom*, under all existing cases — with several cases it opens entirely off-screen and the button reads as dead (the smoke agent clicked it three times before scrolling); render it under the header or scroll it into view on open. (2) Selecting "Вирішено" flips the visible status pill immediately, while the summary confirm (Вирішити/Скасувати) is still pending — whether Скасувати reverts the pill is **unverified**; if it doesn't, the UI lies. (3) The dashboard block's all-clear state drops both the chevron and the badge, so the block silently changes shape between states. One small pass, bundle with the next cases-touching sprint. |
| DEPLOY-RUNBOOK-1 | `SERVER_DEPLOY_PLAN.md` has no recurring-deploy runbook; its §4 compose YAML is stale four ways | Filed from CC's CASE-1 deploy report (2026-08-26). The doc is a first-time bring-up guide — §9 says `git clone`, not `git pull`; the actual recurring sequence (rollback-`<sha>` image tags → pull → `up -d --build` → verify alembic single head) and the **counter-intuitive rollback order** (`alembic downgrade -1` BEFORE swapping images, because the entrypoint runs `upgrade head` on every start and an old image against the new schema crash-loops) exist only inside closure entries in this file. §4's YAML predates cloudflared, the `80:80` removal, the `/app/logs` bind mount and the frontend's switch to `Dockerfile.deploy`. CC followed live server state instead — correctly, but the next agent told to "deploy per SERVER_DEPLOY_PLAN.md" hits the same gap. Fix: add an "§11 Recurring deploy + rollback" runbook and either refresh §4 or stamp it "historical — live state wins". Docs-only. |

---

**Release status (2026-08-04, second release) — read this before trusting anything below**

`main` = **`6e2ec78`**, pushed and **deployed**, single alembic head **`f2b8d6c40a15`**.
**STATEMENT-IMPORT is merged and live on prod** (fast-forward `9552193..6e2ec78`, 28 files; the six
code commits keep their original SHAs). Suites on `main`: **737 backend / 78 MCP / 159 frontend**,
both completeness guards green, reconciliation gate re-run after the merge and still ties to the
cent. Post-deploy: `etsy_statement_line` exists and holds **0 rows**, 677 orders with **0** carrying
a `platform_fee`, 3 shops with **0** rates. **No statement import has been run on prod** — not the
UI, not the MCP tool, not a dry run. That is a separately authorised step.
Rollback target: commit `9552193` (code-identical to `6130eaf`; the difference is docs-only), images
durably tagged `orderhub-{backend,frontend}:rollback-20260804`. Order: `alembic downgrade -1`
(`f2b8d6c40a15` → `e1a4c7b93d28`) **before** swapping images back; never past `c4f1a83e6b27`;
do not `docker image prune`.
Confirmed still healthy after the merge: FX rate 44.7876 present, MCP agent MANAGER with all 3 shop
grants + `view_costs`/`view_finance` (so the statement MCP tool works on prod — see
`AGENT-GRANT-REPLACE`), `order_refunds` still 3 from the SHOPIFY-REFUNDS retro-fix, `shops.fee_percent`
intact.

---

**First release of 2026-08-04 (SHOP-FEE-1) — superseded by the block above, kept for its detail**

`main` was **`6130eaf`**, single alembic head **`e1a4c7b93d28`**. Suites at that point:
