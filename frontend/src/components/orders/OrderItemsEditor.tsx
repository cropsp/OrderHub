        ...
        {items.map((item, idx) => (
          <div key={idx} className="group relative flex flex-wrap sm:flex-nowrap items-end gap-3 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div className="w-20 space-y-2">
               ...
               <Input 
                type="number"
                className="h-9 border-zinc-800 bg-zinc-900/40 rounded-lg text-center text-zinc-100"
                value={item.quantity}
                onChange={e => onUpdateItem(idx, 'quantity', parseInt(e.target.value) || 1)}
              />
            </div>
            <div className="w-32 space-y-2">
               <p className="text-[10px] font-bold uppercase text-zinc-600 px-1 text-right">Price</p>
               <div className="relative">
                  ...
                  />
                  <span className="absolute right-2.5 top-2.5 text-[10px] font-bold text-zinc-600 uppercase">{currency}</span>
               </div>
            </div>
            <Button 
              type="button"
