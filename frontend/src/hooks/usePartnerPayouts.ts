...
/**
 * On-demand, never on list render: each check costs 2-3 aggregates server-side.
 * A mutation rather than a query so it only ever runs when the operator asks.
 */
export function useCheckStaleness(shopId: string) {
  const addToast = useToastStore(s => s.addToast)
  return useMutation({
    mutationFn: (limit?: number) => partnerPayoutsApi.checkStaleness(shopId, limit),
    onError: (error) => {
      addToast(getApiErrorMessage(error, 'Failed to check settlements'), 'error')
    },
  })
}
