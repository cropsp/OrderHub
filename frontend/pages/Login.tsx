...
const Login: React.FC = () => {
  ...
  const [error, setError] = useState(''); // Додано для виведення помилок

  // Зробіть функцію асинхронною і ПЕРЕДАЙТЕ email та password
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');

    try {
      // ✅ ФІКС: Тепер ми передаємо дані для логіну
      await login(email, password); 
    } catch (err) {
      // Обробка помилки, повернутої бекендом (наприклад, "Invalid credentials")
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-background">
      <div className="w-full max-w-md p-8 space-y-8 bg-card rounded-lg shadow-lg">
        ...
        {/* Виведення помилки */}
        {error && (
          <div className="text-sm text-center text-red-500 bg-red-100 p-2 rounded">
            {error}
          </div>
        )}
