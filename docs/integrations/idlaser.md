...
**Status:** describes the state of `main`. Extracted from CLAUDE.md on 2026-08-21 (docs restructuring); content unchanged except the SSE-pattern gotcha, corrected the same day post-extraction (stale `routers/mcp.py` reference).
...
- **SSE pattern**: this is the codebase's first **user-facing** SSE endpoint and uses `sse_starlette.EventSourceResponse` for auto-heartbeat — use it for any new user-facing SSE. (Historical note: an earlier version of this gotcha pointed at the raw `StreamingResponse(media_type="text/event-stream")` in `backend/routers/mcp.py` as "locked"; that router was **deleted** in the 2026-07-27 MCP rebuild, so the anti-pattern no longer exists to copy.)
