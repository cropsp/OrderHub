...
1. **Multi-currency COGS rollup.** Material currency (always UAH today) and Order currency (UAH for KoraKlenu, USD for Lamamarka/Etsy) can mismatch. When that happens, `computed_production_cost` is in the Material's currency (UAH) but `Order.total_price` is in `Order.currency` (USD). Net Profit math in FIN-1 then mixes currencies for that order. **In practice today:** KoraKlenu (UAH orders) → no mismatch. Lamamarka and Etsy shops keep their manual `production_cost` in `Order.currency` until FX conversion ships. Result: Phase 3 improves cost accuracy for KoraKlenu immediately; Lamamarka/Etsy stay on manual `production_cost` even after MAT-5. Not worse than today, just doesn't help.
